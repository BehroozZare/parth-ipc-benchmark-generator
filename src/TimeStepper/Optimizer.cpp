...
extern const std::string csvOutputPath;
...
Optimizer<dim>::Optimizer(const Mesh<dim> &p_data0,
                          ...
                          bool p_mute, const Eigen::MatrixXd &UV_bnds,
                          const Eigen::MatrixXi &E, const Eigen::VectorXi &bnd,
                          const Config &p_animConfig)
    : data0(p_data0), energyTerms(p_energyTerms), energyParams(p_energyParams),
      animConfig(p_animConfig), OSQPSolver(false) {
  ...
  linSysSolver = LinSysSolver<Eigen::VectorXi, Eigen::VectorXd>::create(
      animConfig.linSysSolverType, animConfig.strum_par,
      animConfig.Compression);
  dampingMtr = LinSysSolver<Eigen::VectorXi, Eigen::VectorXd>::create(
      LinSysSolverType::CHOLMOD, animConfig.strum_par,
      animConfig.Compression); // TODO::CHECK FOR OTHER SOLVER

  setAnimScriptType(animConfig.animScriptType, animConfig.meshSeqFolderPath);
  solveWithQP =
      (animConfig.isConstrained && (animConfig.constraintSolverType == CST_QP));
  solveWithSQP = (animConfig.isConstrained &&
                  (animConfig.constraintSolverType == CST_SQP));
  solveIP =
      (animConfig.isConstrained && (animConfig.constraintSolverType == CST_IP));
  if (solveWithQP || solveWithSQP || solveIP) {
    ...
    MMActiveSet.resize(animConfig.meshCollisionObjects.size() +
                       animConfig.isSelfCollision);
    MMActiveSet_next.resize(animConfig.meshCollisionObjects.size() +
                            animConfig.isSelfCollision);
    MMActiveSet_CCD.resize(animConfig.meshCollisionObjects.size() +
                           animConfig.isSelfCollision);
    paraEEMMCVIDSet.resize(animConfig.meshCollisionObjects.size() +
                           animConfig.isSelfCollision);
    paraEEeIeJSet.resize(animConfig.meshCollisionObjects.size() +
                         animConfig.isSelfCollision);
    buildConstraintStartIndsWithMM(activeSet, MMActiveSet, constraintStartInds);
    MMActiveSet_lastH.resize(animConfig.meshCollisionObjects.size() +
                             animConfig.isSelfCollision);
    MMLambda_lastH.resize(animConfig.meshCollisionObjects.size() +
                          animConfig.isSelfCollision);
    MMDistCoord.resize(animConfig.meshCollisionObjects.size() +
                       animConfig.isSelfCollision);
    MMTanBasis.resize(animConfig.meshCollisionObjects.size() +
                      animConfig.isSelfCollision);
    n_collPairs_sum.setZero(animConfig.meshCollisionObjects.size() +
                            animConfig.isSelfCollision);
    n_collPairs_max.setZero(animConfig.meshCollisionObjects.size() +
                            animConfig.isSelfCollision);
    n_convCollPairs_sum.setZero(animConfig.meshCollisionObjects.size() +
                                animConfig.isSelfCollision);
    n_convCollPairs_max.setZero(animConfig.meshCollisionObjects.size() +
                                animConfig.isSelfCollision);
  }
  ...
  animScripter.initAnimScript(result, animConfig.collisionObjects,
                              ...
                              animConfig.DBCTimeRange, animConfig.NBCTimeRange);
  ...
  fricDHatThres = dHat; // enable friction only when collision dHat is smaller
                        // than fricDHatThres if it's not enabled initially
  ...
  for (int vI = 0; vI < V_surf.rows(); ++vI) {
    fprintf(out, "%d: %le %le %le\n", vI + 1, V_surf(vI, 0), V_surf(vI, 1),
            V_surf(vI, 2));
  }
  fprintf(out, "POLYS\n");
  for (int fI = 0; fI < F_surf.rows(); ++fI) {
    int indStart = fI * 3;
    fprintf(out, "%d: %d %d\n", indStart + 1, F_surf(fI, 0) + 1,
            F_surf(fI, 1) + 1);
    fprintf(out, "%d: %d %d\n", indStart + 2, F_surf(fI, 1) + 1,
            F_surf(fI, 2) + 1);
    fprintf(out, "%d: %d %d\n", indStart + 3, F_surf(fI, 2) + 1,
            F_surf(fI, 0) + 1);
  }
  fprintf(out, "END\n");
  fclose(out);
...
template <int dim> Optimizer<dim>::~Optimizer(void) {
...
template <int dim> Mesh<dim> &Optimizer<dim>::getResult(void) { return result; }

template <int dim> int Optimizer<dim>::getIterNum(void) const {
  return globalIterNum;
}
template <int dim> int Optimizer<dim>::getFrameAmt(void) const {
  return frameAmt;
}
template <int dim> int Optimizer<dim>::getInnerIterAmt(void) const {
  return innerIterAmt;
}

template <int dim> void Optimizer<dim>::setRelGL2Tol(double p_relTol) {
  assert(p_relTol > 0.0);
...
template <int dim> double Optimizer<dim>::getDt(void) const { return dt; }
...
template <int dim> void Optimizer<dim>::setTime(double duration, double dt) {
  this->dt = dt;
  dtSq = dt * dt;
...
template <int dim> void Optimizer<dim>::precompute(void) {
...
template <int dim> int Optimizer<dim>::solve(int maxIter) {
  ...
  for (int iterI = 0; iterI < maxIter; iterI++) {
    ...
    if (energyTerms[0]->getNeedElemInvSafeGuard()) {
      if (!result.checkInversion()) {
        spdlog::error(
            "before scripted motion there's element inversion, end process");
        exit(-1);
      }
    }

    if (animScripter.stepAnimScript(result, sh, animConfig.collisionObjects,
                                    animConfig.meshCollisionObjects,
                                    animConfig.ccdMethod, dt, dHat, energyTerms,
                                    animConfig.isSelfCollision,
                                    /*forceIntersectionLineSearch=*/solveIP)) {
      result.saveBCNodes(outputFolderPath + "/BC_" +
                         std::to_string(globalIterNum) + ".txt");
    ...
    for (int coI = 0; coI < animConfig.meshCollisionObjects.size(); ++coI) {
      intersected |=
          !animConfig.meshCollisionObjects[coI]->checkEdgeTriIntersection(
              result, sh);
    }
    if (animConfig.isSelfCollision) {
      intersected |=
          !SelfCollisionHandler<dim>::checkEdgeTriIntersection(result, sh);
    }
    if (intersected) {
      getchar();
    }
    ...
    if (globalIterNum >= frameAmt) {
      ...
      case TIT_BE: {
        ...
        velocity = Eigen::Map<Eigen::VectorXd>(
            RowMatrixXd((result.V - result.V_prev).array() / dt).data(),
            velocity.rows());
        acceleration =
            RowMatrixXd::Map(((velocity - velocity_prev) / dt).eval().data(),
                             acceleration.rows(), acceleration.cols());
        result.V_prev = result.V;
        computeXTilta();
      } break;
      ...
      case TIT_NM:
        dx_Elastic = result.V - xTilta;
        velocity = velocity + dt * (1 - gamma_NM) *
                                  Eigen::Map<Eigen::MatrixXd>(
                                      RowMatrixXd(acceleration).data(),
                                      velocity.rows(), 1);
        acceleration = (result.V - xTilta) / (dtSq * beta_NM);
        acceleration.rowwise() += gravity.transpose();
        velocity += dt * gamma_NM *
                    Eigen::Map<Eigen::MatrixXd>(
                        RowMatrixXd(acceleration).data(), velocity.rows(), 1);
        result.V_prev = result.V;
        computeXTilta();
        break;
      }
...
template <int dim> void Optimizer<dim>::updatePrecondMtrAndFactorize(void) {
  if (!mute) {
  ...
  linSysSolver->set_pattern(solveIP ? vNeighbor_IP : result.vNeighbor,
                            result.DBCVertexIds);
  if (animConfig.dampingStiff) {
    dampingMtr->set_pattern(solveIP ? vNeighbor_IP : result.vNeighbor,
                            result.DBCVertexIds);
  }
  if (!mute) {
    timer_step.start(2);
...
void Optimizer<dim>::precomputeQPObjective(
    ...
    Eigen::SparseMatrix<double> &P, std::vector<double *> &elemPtr_P) const {
...
void Optimizer<dim>::computeQPInequalityConstraint(
    ...
    const std::vector<std::vector<int>> &activeSet, const int num_vars,
    std::vector<int> &constraintStartInds, Eigen::SparseMatrix<double> &A,
  ...
  for (int coI = 0; coI < collisionObjects.size(); ++coI) {
    collisionObjects[coI]->updateConstraints_QP(mesh, activeSet[coI], A_triplet,
                                                b);
  ...
  if (animConfig.isSelfCollision) {
    SelfCollisionHandler<dim>::updateConstraints_QP(
        mesh, MMActiveSet.back(), animConfig.constraintType, mesh_mmcvid_to_toi,
        A_triplet, b);
    constraintStartInds.emplace_back(b.size());
  }
...
bool Optimizer<dim>::solveQP(
    ...
    const LinSysSolver<Eigen::VectorXi, Eigen::VectorXd> *linSys,
    Eigen::SparseMatrix<double> &P, const std::vector<double *> &elemPtr_P,
    Eigen::VectorXd &gradient, OSQP &OSQPSolver,
#ifdef USE_GUROBI
    Eigen::GurobiSparse &gurobiQPSolver,
#endif
    std::vector<int> &constraintStartInds, Eigen::VectorXd &searchDir,
    Eigen::VectorXd &dual, QPSolverType qpSolverType) const {
  // minₓ ½xᵀQx + cᵀx
  if (!solveWithQP) {
  ...
  Eigen::VectorXd b;
  this->computeQPInequalityConstraint(mesh, collisionObjects, activeSet,
                                      linSys->getNumRows(), constraintStartInds,
                                      A, b);

  switch (qpSolverType) {
  case QPSolverType::QP_OSQP:
    return solveQP_OSQP(P, gradient, A, b, OSQPSolver, searchDir, dual);
  case QPSolverType::QP_GUROBI:
#ifdef USE_GUROBI
    return solveQP_Gurobi(P, gradient, A, b, gurobiQPSolver, searchDir, dual);
#else
    spdlog::error("Gurobi QP solve is disabled. Set IPC_WITH_GUROBI=ON in "
                  "cmake to enable.");
    throw std::runtime_error("Gurobi QP solve is disabled. Set "
                             "IPC_WITH_GUROBI=ON in cmake to enable.");
#endif
  }

  // std::string folderPath = outputFolderPath + std::to_string(globalIterNum) +
  // "/"; mkdir(folderPath.c_str(), 0777);
  // IglUtils::writeSparseMatrixToFile(folderPath + "P" +
  // std::to_string(innerIterAmt), Q, true);
  // IglUtils::writeVectorToFile(folderPath + "q" +
  // std::to_string(innerIterAmt), c);
  // IglUtils::writeSparseMatrixToFile(folderPath + "A" +
  // std::to_string(innerIterAmt), A, true);
  // IglUtils::writeVectorToFile(folderPath + "l" +
  // std::to_string(innerIterAmt), b); IglUtils::writeVectorToFile(folderPath +
  // "primal" + std::to_string(innerIterAmt), x);
  // IglUtils::writeVectorToFile(folderPath + "dual" +
  // std::to_string(innerIterAmt), dual);
}

template <int dim>
bool Optimizer<dim>::solveQP_OSQP(Eigen::SparseMatrix<double> &Q,
                                  Eigen::VectorXd &c,
                                  Eigen::SparseMatrix<double> &A,
                                  Eigen::VectorXd &b, OSQP &QPSolver,
                                  Eigen::VectorXd &x,
  ...
  QPSolver.setup(Q.valuePtr(), c_int(Q.nonZeros()), Q.innerIndexPtr(),
                 Q.outerIndexPtr(), c.data(), A.valuePtr(), c_int(A.nonZeros()),
                 A.innerIndexPtr(), A.outerIndexPtr(), b.data(), u.data(),
                 num_vars, num_constraints);

  x.conservativeResize(num_vars);
  c_int status = QPSolver.solve();
  if ((status != -3) && (status != -4)) {
    memcpy(x.data(), QPSolver.getPrimal(), x.size() * sizeof(x[0]));
  }

  if (num_constraints > 0) {
    dual.conservativeResize(num_constraints);
    if ((status != -3) && (status != -4)) {
      memcpy(dual.data(), QPSolver.getDual(), dual.size() * sizeof(dual[0]));
    }
  }
...
#ifdef USE_GUROBI
template <int dim>
bool Optimizer<dim>::solveQP_Gurobi(const Eigen::SparseMatrix<double> &Q,
  ...
  Eigen::SparseVector<double> inf =
      Eigen::VectorXd::Constant(num_vars,
  ...
  try {
    success = QPSolver.solve(
        /*Q=*/Q, /*C=*/c.sparseView(),
        /*Aeq=*/Eigen::SparseMatrix<double>(),
        /*Beq=*/Eigen::SparseVector<double>(),
...
template <int dim>
void Optimizer<dim>::computeQPResidual(
    const Mesh<dim> &mesh,
    const std::vector<std::shared_ptr<CollisionObject<dim>>> &collisionObjects,
    const std::vector<std::vector<int>> &activeSet,
    const std::vector<int> &constraintStartInds,
    const Eigen::VectorXd &gradient, const Eigen::VectorXd &dual,
    Eigen::VectorXd &grad_KKT, Eigen::VectorXd &constraintVal,
    Eigen::VectorXd &fb) {
  grad_KKT = gradient;
  constraintVal.conservativeResize(0);
  for (int coI = 0; coI < collisionObjects.size(); ++coI) {
    int constraintAmtI =
        constraintStartInds[coI + 1] - constraintStartInds[coI];
    if (constraintAmtI) {
      collisionObjects[coI]->evaluateConstraintsQP(mesh, activeSet[coI],
                                                   constraintVal);
      collisionObjects[coI]->leftMultiplyConstraintJacobianTQP(
          mesh, activeSet[coI],
          dual.segment(constraintStartInds[coI], constraintAmtI), grad_KKT);
    }
  }

  for (int coI = 0; coI < animConfig.meshCollisionObjects.size(); ++coI) {
    int constraintAmtI =
        constraintStartInds[coI + 1 + collisionObjects.size()] -
        constraintStartInds[coI + collisionObjects.size()];
    if (constraintAmtI) {
      ...
      animConfig.meshCollisionObjects[coI]->leftMultiplyConstraintJacobianTQP(
          ...
          dual.segment(constraintStartInds[coI + collisionObjects.size()],
                       constraintAmtI),
          animConfig.constraintType, grad_KKT);
    }
  }
  ...
  fb.conservativeResize(
      constraintStartInds
          .back()); // Fischer-Burmeister measure for complementarity
  for (int constraintI = 0; constraintI < constraintStartInds.back();
       ++constraintI) {
    const double dualI =
        -dual[constraintI]; // OSQP produce negative dual for >= constraints
    const double &constraint = constraintVal[constraintI];
    fb[constraintI] = (dualI + constraint -
...
void Optimizer<dim>::initX(int option,
                           std::vector<std::vector<int>> &p_activeSet_next) {
  // global:
...
#ifdef USE_TBB
    tbb::parallel_for(0, (int)result.V.rows(), 1,
                      [&](int vI)
#else
...
#endif
                      {
                        if (result.isDBCVertex(vI)) {
                          ...
                          searchDir.segment<dim>(vI * dim) =
                              dt * velocity.segment<dim>(vI * dim);
                        }
                      }
#ifdef USE_TBB
    );
      ...
      for (int vI = 0; vI < result.V.rows(); vI++)
#endif
                        {
                          if (result.isDBCVertex(vI)) {
                            ...
                            searchDir.segment<dim>(vI * dim) =
                                (dt * velocity.segment<dim>(vI * dim) +
                                 gravityDtSq + dx_Elastic.row(vI).transpose());
                          }
                        }
#ifdef USE_TBB
      );
...
#endif
                        {
                          if (result.isDBCVertex(vI)) {
                            ...
                            searchDir.segment<dim>(vI * dim) =
                                dt * velocity.segment<dim>(vI * dim) +
                                gravityDtSq / 2.0 +
                                dx_Elastic.row(vI).transpose() * 2.0;
                          }
                        }
#ifdef USE_TBB
      );
      ...
      for (int vI = 0; vI < result.V.rows(); vI++)
#endif
          {
            if (result.isDBCVertex(vI)) {
              ...
              searchDir.segment<dim>(vI * dim) =
                  (dt * velocity.segment<dim>(vI * dim) +
                   (gravityDtSq + 0.5 * dx_Elastic.row(vI).transpose()));
            }
          }
#ifdef USE_TBB
      );
    ...
    for (int vI = 0; vI < result.V.rows(); vI++)
#endif
        {
          if (result.isDBCVertex(vI)) {
            ...
            searchDir[vI * dim] =
                -g[vI * dim] / linSysSolver->coeffMtr(vI * dim, vI * dim);
            searchDir[vI * dim + 1] =
                -g[vI * dim + 1] /
                linSysSolver->coeffMtr(vI * dim + 1, vI * dim + 1);
            searchDir[vI * dim + 2] =
                -g[vI * dim + 2] /
                linSysSolver->coeffMtr(vI * dim + 2, vI * dim + 2);
          }
        }
#ifdef USE_TBB
    );
  ...
  default:
    spdlog::warn(
        "unkown primal initialization type, use last timestep instead");
    break;
  }
  ...
  if (option) {
    ...
    if (solveWithQP || solveWithSQP) {
      ...
      for (int coI = 0; coI < animConfig.collisionObjects.size(); coI++) {
        ...
        animConfig.collisionObjects[coI]->largestFeasibleStepSize(
            result, searchDir, slackness_a, p_activeSet_next[coI], stepSize);
      }
      ...
      for (int coI = 0; coI < animConfig.meshCollisionObjects.size(); coI++) {
        ...
        case ccd::CCDMethod::TIGHT_INCLUSION:
          ...
          break;
#else
          spdlog::error("Tight Inclusion CCD is disabled in CMake "
                        "(CCD_WRAPPER_WITH_TIGHT_INCLUSION=OFF)!");
          throw std::runtime_error("Tight Inclusion CCD is disabled in CMake "
                                   "(CCD_WRAPPER_WITH_TIGHT_INCLUSION=OFF)!");
#endif
      ...
      if (animConfig.isSelfCollision) {
        ...
        case ccd::CCDMethod::TIGHT_INCLUSION:
          ...
          SelfCollisionHandler<dim>::largestFeasibleStepSize_CCD_TightInclusion(
              result, sh, searchDir, animConfig.ccdTolerance, newCandidates,
              stepSize);
          break;
#else
          spdlog::error("Tight Inclusion CCD is disabled in CMake "
    ...
    if (!(solveWithQP || solveWithSQP)) {
      ...
      if (energyTerms[0]->getNeedElemInvSafeGuard()) {
        while (!result.checkInversion(true)) {
          logFile << "element inversion detected during warmStart, backtrack!"
                  << std::endl;
          stepSize /= 2.0;
          stepForward(V0, result, stepSize);
        }
      ...
      if (solveIP) {
        ...
        while (isIntersected(result, sh, V0, animConfig)) {
          logFile << "intersection detected during warmStart, backtrack!"
                  << std::endl;
          stepSize /= 2.0;
          stepForward(V0, result, stepSize);
          sh.build(result, result.avgEdgeLen / 3.0);
    ...
    spdlog::debug("timestep={:d} init_stepSize={:g}", globalIterNum, stepSize);
  } else if (solveWithQP || solveWithSQP || solveIP) {
    clearActiveSet();
  }
}

template <int dim> void Optimizer<dim>::computeXTilta(void) {
    ...
    for (int vI = 0; vI < result.V.rows(); vI++)
#endif
        {
          if (result.isDBCVertex(vI)) {
            ...
            xTilta.row(vI) =
                (result.V_prev.row(vI) +
                 (velocity.segment<dim>(vI * dim) * dt + gravityDtSq)
                     .transpose());
          }
        }
#ifdef USE_TBB
    ...
    for (int vI = 0; vI < result.V.rows(); vI++)
#endif
        {
          if (result.isDBCVertex(vI)) {
            ...
            xTilta.row(vI) =
                (result.V_prev.row(vI) +
                 (velocity.segment<dim>(vI * dim) * dt + beta_NM * gravityDtSq +
                  (0.5 - beta_NM) * (dtSq * acceleration.row(vI).transpose()))
                     .transpose());
          }
        }
#ifdef USE_TBB
...
template <int dim> void Optimizer<dim>::clearActiveSet() {
  for (int coI = 0; coI < animConfig.collisionObjects.size(); coI++) {
...
template <int dim> bool Optimizer<dim>::updateActiveSet_QP() {
  ...
  for (int coI = 0; coI < animConfig.collisionObjects.size(); ++coI) {
    ...
    animConfig.collisionObjects[coI]->filterSearchDir_QP(result, searchDir,
                                                         activeSet_next[coI]);
    activeSet[coI] = activeSet_next[coI];
  }
  for (int coI = 0; coI < animConfig.meshCollisionObjects.size(); coI++) {
    newConstraintsAdded |=
        animConfig.meshCollisionObjects[coI]->updateActiveSet_QP(
            result, searchDir, animConfig.constraintType, MMActiveSet_next[coI],
            animConfig.ccdMethod, eta, animConfig.ccdTolerance);
    MMActiveSet[coI] = MMActiveSet_next[coI];
  }
  if (animConfig.isSelfCollision) {
    newConstraintsAdded |= SelfCollisionHandler<dim>::updateActiveSet_QP(
        result, searchDir, animConfig.constraintType, MMActiveSet_next.back(),
        mesh_mmcvid_to_toi, animConfig.ccdMethod, eta, animConfig.ccdTolerance);
    MMActiveSet.back() = MMActiveSet_next.back();
  }
  return newConstraintsAdded;
}

template <int dim> int Optimizer<dim>::countConstraints() {
  int constraintAmt = 0;
  for (int coI = 0; coI < animConfig.collisionObjects.size(); ++coI) {
...
template <int dim> bool Optimizer<dim>::fullyImplicit(void) {
  timer_step.start(10);

  // initial warm start with inital guess search direction p₀
  // and compute initial active set A_0 checking for collisions from x_t to x_t
  // + p_0
  ...
  double meshScale =
      (result.V.colwise().maxCoeff() - result.V.colwise().minCoeff())
          .maxCoeff();
  ...
  do {
    ...
    if (solve_oneStep()) {
      spdlog::error("timestep={:d} iteration={:d} "
    ...
    if (searchDir.norm() > 100 * meshScale) {
      spdlog::warn("timestep={:d} iteration={:05d} msg=\"possible displacement "
                   "blow-up\" meshScale={:g} searchDir.norm()={:g}",
                   globalIterNum, curIterI, meshScale, searchDir.norm());
      result.saveSurfaceMesh("output/blowup_t" + std::to_string(globalIterNum) +
                             "_i" + std::to_string(curIterI) + ".obj");
    ...
    if (solveWithQP || solveWithSQP) {
      computeQPResidual(result, animConfig.collisionObjects, activeSet,
                        constraintStartInds, gradient, dual_QP, grad_KKT,
                        constraintVal, fb);
      // || H(xᵗ)dx + ∇E(xᵗ) + Cᵀλ ||²
      if (solveWithQP) {
    ...
    file_iterStats << fmt::format(
        "{:g} {:g} {:d}\n", lastEnergyVal, sqn_g,
        (constraintStartInds.empty() ? 0 : constraintStartInds.back()));
    timer_step.stop();

    constraintAmt = countConstraints();
    double currentEnergyVal;
    computeEnergyVal(result, true, currentEnergyVal);
    spdlog::debug("timestep={:d} iteration={:05d} currentEnergyVal={:.6e} "
                  "gradient.squaredNorm()={:g} sqn_g={:.6e} fb.norm()={:.6e} "
                  "constraintAmt={:03d} newConstraintsAdded={}",
                  globalIterNum, curIterI, currentEnergyVal,
                  gradient.squaredNorm(), sqn_g, fb.norm(), constraintAmt,
                  newConstraintsAdded);

    if (++curIterI >= iterCap) {
      spdlog::warn("timestep={:d} msg=\"optimization did not converge in at "
                   "most {:d} iterations\" "
                   "sqn_g={:g} targetGRes={:g} fb.norm()={:g} fbNormTol={:g}",
                   globalIterNum, iterCap, sqn_g, targetGRes, fb.norm(),
                   fbNormTol);
      break;
    }
    ...
    if ((curEnergyVal - lastEnergyVal) / lastEnergyVal > 1e9 &&
        curEnergyVal > 1e9) {
      spdlog::warn("timestep={:d} iteration={:05d} msg=\"possible energy "
                   "blow-up\" lastEnergyVal={:g} curEnergyVal={:g}",
    ...
    if ((sqn_g - sqn_g0) / sqn_g0 > 1e9 && sqn_g > 1e9) {
      spdlog::warn("timestep={:d} iteration={:05d} msg=\"possible gradient "
                   "blow-up\" sqn_g0={:g} sqn_g={:g}",
  ...
  for (int coI = 0; coI < animConfig.meshCollisionObjects.size(); ++coI) {
    if (!animConfig.meshCollisionObjects[coI]->checkEdgeTriIntersection(result,
                                                                        sh)) {
      spdlog::error("timestep={:d} msg=\"intersecting state of the mesh and "
                    "collision object {:d}\"",
                    globalIterNum, coI);
      isIntersecting = true;
    }
  ...
  if (animConfig.isSelfCollision &&
      !SelfCollisionHandler<dim>::checkEdgeTriIntersection(result, sh)) {
    spdlog::error("timestep={:d} msg=\"self-intersecting state\"",
                  globalIterNum);
    isIntersecting = true;
  }
  if (isIntersecting) {
    result.saveSurfaceMesh("output/intersecting_t" +
                           std::to_string(globalIterNum) + ".obj");
#ifdef EXIT_UPON_QP_FAIL
    exit(1);
#endif
  }
...
template <int dim> bool Optimizer<dim>::fullyImplicit_IP(void) {
  ...
  for (int coI = 0; coI < animConfig.collisionObjects.size(); ++coI) {
    ...
    animConfig.collisionObjects[coI]->evaluateConstraints(
        result, activeSet[coI], constraintVal);
    ...
    if (animConfig.collisionObjects[coI]->friction > 0.0) {
      ...
      for (int i = 0; i < lambda_lastH[coI].size(); ++i) {
        ...
        lambda_lastH[coI][i] *=
            -kappa * 2.0 * std::sqrt(constraintVal[startCI + i]);
      }
  ...
  for (int coI = 0; coI < animConfig.meshCollisionObjects.size(); ++coI) {
    animConfig.meshCollisionObjects[coI]->evaluateConstraints(
        result, MMActiveSet[coI], constraintVal);
  }
  if (animConfig.isSelfCollision) {
    int startCI = constraintVal.size();
    SelfCollisionHandler<dim>::evaluateConstraints(result, MMActiveSet.back(),
                                                   constraintVal);
  ...
  timer_step.start(10);
  computeEnergyVal(result, true,
                   lastEnergyVal); // for possible warmstart and pre-scripting
  ...
  while (true) {
    ...
    if (animConfig.isSelfCollision) {
      ...
      if (animConfig.selfFric > 0.0) {
        ...
        // TODO: parallelize
        for (int i = 0; i < MMLambda_lastH.back().size(); ++i) {
          compute_g_b(constraintVal[startCI + i], dHat,
                      MMLambda_lastH.back()[i]);
          MMLambda_lastH.back()[i] *=
              -kappa * 2.0 * std::sqrt(constraintVal[startCI + i]);
          if (MMActiveSet.back()[i][3] < -1) {
    ...
    if (constraintVal.size()) {
      logFile << "d range: [" << constraintVal.minCoeff() << ", "
              << constraintVal.maxCoeff() << "]" << std::endl;
      ...
      if (constraintVal.maxCoeff() < dHatTarget) {
        logFile << "Discrete complimentarity slackness condition satisfied"
                << std::endl;
        updateDHat = false;
      ...
      if (solveFric) {
        ...
        if (fricDHat <= fricDHatTarget) {
          ...
          if (animConfig.fricIterAmt > 0 &&
              fricIterI >= animConfig.fricIterAmt) {
            updateFricDHat = false;
          }
        }
      } else {
    ...
    if constexpr (HOMOTOPY_VAR == 0) {
      ...
      spdlog::error(
          "needs to define HOMOTOPY_VAR to either 0 (kappa) or 1 (dHat)");
      exit(-1);
    }
  }
...
template <int dim>
bool Optimizer<dim>::solveSub_IP(double kappa,
                                 std::vector<std::vector<int>> &AHat,
                                 std::vector<std::vector<MMCVID>> &MMAHat) {
  std::string Data_name = outputFolderPath + animConfig.sim_name;
  linSysSolver->csv_data_address = Data_name;
  int iterCap = 10000,
      k = 0; // 10000 for running comparative schemes that can
             // potentially not converge
  m_projectDBC = true;
  rho_DBC = 0.0;

  linSysSolver->outputFolderPath = outputFolderPath;

  double prev_load_time = 0;
  double prev_analyze_time = 0;
  double prev_factor_time = 0;
  double prev_solve_time = 0;

  double lastMove = animScripter.getCompletedStepSize();
  double last_energy_val = 0;

  for (; k < iterCap; ++k) {

    double iter_time = omp_get_wtime();

    last_energy_val = lastEnergyVal;
    linSysSolver->frame = globalIterNum;
    linSysSolver->iteration = k;
    ...
    spdlog::info("# constraint = {:d}",
                 MMActiveSet.size() ? MMActiveSet.back().size() : 0);
    spdlog::info("# paraEE = {:d}",
                 paraEEMMCVIDSet.size() ? paraEEMMCVIDSet.back().size() : 0);

    // check convergence
    double gradSqNorm = gradient.squaredNorm();
    double distToOpt_PN = searchDir.cwiseAbs().maxCoeff();
    spdlog::info(
        "kappa = {:g}, dHat = {:g}, eps_v = {:g}, subproblem ||g||^2 = {:g}",
        kappa, dHat, fricDHat, gradSqNorm);
    spdlog::info("distToOpt_PN = {:g}, targetGRes = {:g}", distToOpt_PN,
                 targetGRes);
    bool gradVanish = (distToOpt_PN < targetGRes);
    if (!useGD && k && gradVanish &&
        (animScripter.getCompletedStepSize() > 1.0 - 1.0e-3) &&
        (animScripter.getCOCompletedStepSize() > 1.0 - 1.0e-3)) {
      // subproblem converged
      logFile << k << " Newton-type iterations for kappa = " << kappa
              << ", dHat = " << dHat << std::endl;
      return false;
    }
    innerIterAmt++;
    ...
    for (int coI = 0; coI < animConfig.collisionObjects.size(); ++coI) {
      animConfig.collisionObjects[coI]->largestFeasibleStepSize(
          result, searchDir, slackness_a, AHat[coI], alpha);
    }
    ...
    for (int coI = 0; coI < animConfig.meshCollisionObjects.size(); ++coI) {
      ...
      case ccd::CCDMethod::TIGHT_INCLUSION:
        ...
        animConfig.meshCollisionObjects[coI]
            ->largestFeasibleStepSize_TightInclusion(
                result, sh, searchDir, animConfig.ccdTolerance,
      ...
      default:
        animConfig.meshCollisionObjects[coI]->largestFeasibleStepSize_exact(
            result, sh, searchDir, animConfig.ccdMethod, MMActiveSet_CCD[coI],
            alpha);
        break;
      }
    }
    ...
    if (animConfig.isSelfCollision) {
      ...
      case ccd::CCDMethod::FLOATING_POINT_ROOT_FINDER:
        SelfCollisionHandler<dim>::largestFeasibleStepSize(
            result, sh, searchDir, slackness_m, MMActiveSet_CCD.back(),
            newCandidates, alpha);
        break;
      ...
      case ccd::CCDMethod::TIGHT_INCLUSION:
        ...
        SelfCollisionHandler<dim>::largestFeasibleStepSize_TightInclusion(
            result, sh, searchDir, animConfig.ccdTolerance,
            MMActiveSet_CCD.back(), newCandidates, alpha);
        break;
#else
        spdlog::error("Tight Inclusion CCD is disabled in CMake "
                      "(CCD_WRAPPER_WITH_TIGHT_INCLUSION=OFF)!");
      ...
      default:
        SelfCollisionHandler<dim>::largestFeasibleStepSize_exact(
            result, sh, searchDir, animConfig.ccdMethod, MMActiveSet_CCD.back(),
            alpha);
        break;
      }
    }
    ...
    if (MMActiveSet_CCD
            .size()) { // if there is mesh-mesh collision (CO or selfCollision)
      ...
      if constexpr (CFL_FOR_CCD == 1) {
        ...
        if ((!k && alpha > alpha_CFL) || alpha > 2.0 * alpha_CFL) {
          ...
          timer_temp3.stop();
#endif
          for (int coI = 0; coI < animConfig.meshCollisionObjects.size();
               ++coI) {
            switch (animConfig.ccdMethod) {
            case ccd::CCDMethod::FLOATING_POINT_ROOT_FINDER:
              animConfig.meshCollisionObjects[coI]->largestFeasibleStepSize_CCD(
            ...
            case ccd::CCDMethod::TIGHT_INCLUSION:
              ...
              throw std::runtime_error(
                  "Tight Inclusion CCD is disabled in CMake "
                  "(CCD_WRAPPER_WITH_TIGHT_INCLUSION=OFF)!");
#endif
          ...
          if (alpha < alpha_CFL) {
            alpha = alpha_CFL;
            spdlog::info("take a full CCD instead but failed, take CFL {:g}",
                         alpha);
          } else {
    ...
    if (alpha_feasible < 0.25) {
      static int count = 0;
      Eigen::MatrixXd V0 = result.V;
      result.saveSurfaceMesh(
          fmt::format("{}lineSearch-{:05d}-t0.obj", outputFolderPath, count));
      stepForward(V0, result, alpha_feasible);
      result.saveSurfaceMesh(fmt::format("{}lineSearch-{:05d}-tα={:g}.obj",
                                         outputFolderPath, count,
                                         alpha_feasible));
      stepForward(V0, result, 1);
      result.saveSurfaceMesh(
          fmt::format("{}lineSearch-{:05d}-t1.obj", outputFolderPath, count));
      result.V = V0;
      count++;
    }
#endif
    ...
    if (alpha_feasible < 1.0e-8) {
      logFile << "tiny feasible step size " << alpha_feasible << " " << alpha
              << std::endl;
    } else if (alpha < 1.0e-8) {
      logFile << "tiny step size after armijo " << alpha_feasible << " "
              << alpha << std::endl;
    }

    if (linSysSolver->type() == LinSysSolverType::AMGCL &&
        alpha < 1.0e-8) { // only for debugging tiny step size issue
      // output system in AMGCL format
      // Eigen::VectorXd minusG = -gradient;
      // dynamic_cast<AMGCLSolver<Eigen::VectorXi,
      // Eigen::VectorXd>*>(linSysSolver)->write_AMGCL("Stiff", minusG);
      // exit(-1);
      //
      ...
      // for (int j = 0; j < 4; ++j) {
      //     printf("%.20le %.20le %.20le\n",
      //         searchDir[i[j] * dim], searchDir[i[j] * dim + 1],
      //         searchDir[i[j] * dim + 2]);
      // }
      ...
      //     1000.0 * largestFeasibleStepSize,
      //         energies[0], energies[1], energies[2], energies[3], E);
      //     if (i == -1) {
      //         for (int j = -999; j < 0; ++j) {
      //             stepForward(Vlast, result, j / 1000000.0 *
      //             largestFeasibleStepSize); double E;
      //             computeConstraintSets(result);
      //             computeEnergyVal(result, true, E);
      //             fprintf(out, "%.20le %.20le %.20le %.20le %.20le %.20le\n",
      ...
      //         if (cI[0] >= 0) { // EE
      //             const Eigen::Matrix<double, 1, dim> e01 =
      //             result.V.row(cI[1]) - result.V.row(cI[0]); const
      //             Eigen::Matrix<double, 1, dim> e23 = result.V.row(cI[3]) -
      //             result.V.row(cI[2]); double sine =
      //             std::sqrt(e01.cross(e23).squaredNorm() / (e01.squaredNorm()
      //             * e23.squaredNorm())); if (sine < 0.02) {
      //                 spdlog::info("{:d}-{:d} {:d}-{:d} sine = {:g}", cI[0],
      //                 cI[1], cI[2], cI[3], sine);
      //             }
      //         }
      //     }
      //     spdlog::info("cosine p g = {:g}", searchDir.dot(gradient) /
      //     std::sqrt(searchDir.squaredNorm() * gradient.squaredNorm()));
      // }
    } else {
      useGD = false;
    }
    ...
    linSysSolver->addCSVRecord(Data_name, globalIterNum, k, iter_time);
  }

  if (k >= iterCap) {
    logFile << "iteration cap reached for IP subproblem solve!!!" << std::endl;
    spdlog::error("iteration cap reached for IP subproblem solve!!!");
    exit(0); // for stopping comparative schemes that can potentially not
             // converge
  }
...
template <int dim> void Optimizer<dim>::upperBoundKappa(double &kappa) {
  double H_b;
  compute_H_b(1.0e-16 * bboxDiagSize2, dHat, H_b);
  double kappaMax = 100 * animConfig.kappaMinMultiplier *
                    result.avgNodeMass(dim) / (4.0e-16 * bboxDiagSize2 * H_b);
  if (kappa > kappaMax) {
    kappa = kappaMax;
    logFile << "upper bounded kappa to " << kappaMax << " at dHat = " << dHat
            << std::endl;
  }
}

template <int dim> void Optimizer<dim>::suggestKappa(double &kappa) {
  double H_b;
  compute_H_b(1.0e-16 * bboxDiagSize2, dHat, H_b);
  kappa = animConfig.kappaMinMultiplier * result.avgNodeMass(dim) /
          (4.0e-16 * bboxDiagSize2 * H_b);
}

template <int dim> void Optimizer<dim>::initKappa(double &kappa) {
  ...
  if (constraintStartInds.back()) {
    ...
    for (int coI = 0; coI < animConfig.meshCollisionObjects.size(); ++coI) {
      animConfig.meshCollisionObjects[coI]->evaluateConstraints(
          result, MMActiveSet[coI], constraintVal);
      int startCI =
          constraintStartInds[coI + animConfig.collisionObjects.size()];
      for (int cI = startCI;
           cI <
           constraintStartInds[coI + 1 + animConfig.collisionObjects.size()];
           ++cI) {
        compute_g_b(constraintVal[cI], dHat, constraintVal[cI]);
      }
      animConfig.meshCollisionObjects[coI]->leftMultiplyConstraintJacobianT(
          result, MMActiveSet[coI],
          constraintVal.segment(startCI, MMActiveSet[coI].size()), g_c);
    }
    if (animConfig.isSelfCollision) {
      SelfCollisionHandler<dim>::evaluateConstraints(result, MMActiveSet.back(),
                                                     constraintVal);
      int startCI = constraintStartInds[animConfig.meshCollisionObjects.size() +
                                        animConfig.collisionObjects.size()];
      for (int cI = startCI; cI < constraintVal.size(); ++cI) {
        compute_g_b(constraintVal[cI], dHat, constraintVal[cI]);
      }
      SelfCollisionHandler<dim>::leftMultiplyConstraintJacobianT(
          result, MMActiveSet.back(),
...
template <int dim> void Optimizer<dim>::initSubProb_IP(void) {
...
void Optimizer<dim>::computeSearchDir(int k, bool projectDBC) {
  // compute matrix
  //    Eigen::MatrixXd Test = result.V;
  //    result.V = result.V_rest;
  ...
  const static Eigen::IOFormat CSVFormat(Eigen::FullPrecision,
                                         Eigen::DontAlignCols, ", ", "\n");
  std::ofstream file(outputFolderPath + "/V_" + std::to_string(globalIterNum) +
                     "_" + std::to_string(k));
  if (file.is_open()) {
    file << result.V.format(CSVFormat);
    file.close();
  }

  //    f_name = outputFolderPath + "/L_" + uniq_f_name
  //        + "_IPC.mtx";
  //    linSysSolver->outputFactorization(f_name);
  ...
  if (useGD || !linSysSolver->factorize()) {
    ...
    // IglUtils::writeSparseMatrixToFile(outputFolderPath + "H_IP",
    // linSysSolver, true); std::cout << "writing complete, terminate program"
    // << std::endl; exit(0);
...
template <int dim> void Optimizer<dim>::postLineSearch(double alpha) {
#ifdef ADAPTIVE_KAPPA
  if (kappa == 0.0) {
    initKappa(kappa);
  } else {
    ...
    for (int i = 0; i < closeConstraintID.size(); ++i) {
      double d;
      animConfig.collisionObjects[closeConstraintID[i].first]
          ->evaluateConstraint(result, closeConstraintID[i].second, d);
    ...
    if (!updateKappa) {
      for (int i = 0; i < closeMConstraintID.size(); ++i) {
        double d;
        if (closeMConstraintID[i].first <
            animConfig.meshCollisionObjects.size()) {
          animConfig.meshCollisionObjects[closeMConstraintID[i].first]
              ->evaluateConstraint(result, closeMConstraintID[i].second, d);
        } else {
          SelfCollisionHandler<dim>::evaluateConstraint(
              result, closeMConstraintID[i].second, d);
        }
    ...
    for (int coI = 0; coI < animConfig.collisionObjects.size(); ++coI) {
      ...
      for (int i = 0; i < activeSet[coI].size(); ++i) {
        if (constraintVal[constraintValIndStart + i] < dTol) {
          ...
          closeConstraintVal.emplace_back(
              constraintVal[constraintValIndStart + i]);
        }
      }
    }
    for (int coI = 0; coI < animConfig.meshCollisionObjects.size(); ++coI) {
      ...
      for (int i = 0; i < MMActiveSet[coI].size(); ++i) {
        // std::cout << MMActiveSet[coI][i][0] << " " << MMActiveSet[coI][i][1]
        // << " " << MMActiveSet[coI][i][2] << " " << MMActiveSet[coI][i][3] <<
        // ", d=" << constraintVal[constraintValIndStart + i] << std::endl;
        if (constraintVal[constraintValIndStart + i] < dTol) {
          closeMConstraintID.emplace_back(coI, MMActiveSet[coI][i]);
          closeMConstraintVal.emplace_back(
              constraintVal[constraintValIndStart + i]);
        }
      }
    }
    if (animConfig.isSelfCollision) {
      ...
      for (int i = 0; i < MMActiveSet.back().size(); ++i) {
        // std::cout << MMActiveSet.back()[i][0] << " " <<
        // MMActiveSet.back()[i][1] << " " << MMActiveSet.back()[i][2] << " " <<
        // MMActiveSet.back()[i][3] << ", d=" <<
        // constraintVal[constraintValIndStart + i] << std::endl;
        if (constraintVal[constraintValIndStart + i] < dTol) {
          closeMConstraintID.emplace_back(MMActiveSet.size() - 1,
                                          MMActiveSet.back()[i]);
          closeMConstraintVal.emplace_back(
              constraintVal[constraintValIndStart + i]);
        }
...
#endif
  for (int coI = 0; coI < animConfig.collisionObjects.size(); ++coI) {
    animConfig.collisionObjects[coI]->computeConstraintSet(data, dHat,
                                                           activeSet[coI]);
  }
  for (int coI = 0; coI < animConfig.meshCollisionObjects.size(); ++coI) {
    animConfig.meshCollisionObjects[coI]->computeConstraintSet(
        data, sh, dHat, MMActiveSet[coI], paraEEMMCVIDSet[coI],
        paraEEeIeJSet[coI], CFL_FOR_CCD, MMActiveSet_CCD[coI]);
  }
  if (animConfig.isSelfCollision) {
    SelfCollisionHandler<dim>::computeConstraintSet(
        data, sh, dHat, MMActiveSet.back(), paraEEMMCVIDSet.back(),
        paraEEeIeJSet.back(), CFL_FOR_CCD, MMActiveSet_CCD.back());
  }
  timer_step.stop();
}

template <int dim>
void Optimizer<dim>::buildConstraintStartInds(
    const std::vector<std::vector<int>> &activeSet,
    std::vector<int> &constraintStartInds) {
  constraintStartInds.resize(1);
  constraintStartInds[0] = 0;
  for (int coI = 0; coI < animConfig.collisionObjects.size(); ++coI) {
    constraintStartInds.emplace_back(constraintStartInds.back() +
                                     activeSet[coI].size());
  }
}
template <int dim>
void Optimizer<dim>::buildConstraintStartIndsWithMM(
    const std::vector<std::vector<int>> &activeSet,
  ...
  for (int coI = 0; coI < animConfig.meshCollisionObjects.size(); ++coI) {
    constraintStartInds.emplace_back(constraintStartInds.back() +
                                     MMActiveSet[coI].size());
  }
  if (animConfig.isSelfCollision) {
    constraintStartInds.emplace_back(constraintStartInds.back() +
                                     MMActiveSet.back().size());
  }
}

template <int dim> bool Optimizer<dim>::solve_oneStep(void) {
  if (!solveWithQP) {
    ...
    try {
      ...
      IglUtils::writeSparseMatrixToFile(outputFolderPath + "mtr_numFacFail",
                                        linSysSolver, true);
      spdlog::error("numerical factorization failed, matrix written into "
                    "{}mtr_numFacFail",
                    outputFolderPath);
      exit(-1);
    }
  ...
  if (solveWithQP || solveWithSQP) {
    ...
    solveQPSuccess = solveQP(
        result, animConfig.collisionObjects, activeSet, linSysSolver, P_QP,
        elemPtr_P_QP, gradient, OSQPSolver,
#ifdef USE_GUROBI
        gurobiQPSolver,
#endif
        constraintStartInds, searchDir, dual_QP, animConfig.qpSolverType);

#ifdef USE_GUROBI
    if (animConfig.qpSolverType == QP_GUROBI) {
      if (solveQPSuccess) {
        spdlog::debug("timestep={:d} Gurobi_status={:d} description=\"{:s}\"",
                      globalIterNum, gurobiQPSolver.status(),
                      gurobiQPSolver.statusDescription());
      } else {
        spdlog::error("timestep={:d} Gurobi_status={:d} description=\"{:s}\"",
                      globalIterNum, gurobiQPSolver.status(),
                      gurobiQPSolver.statusDescription());
      }
    }
...
#ifdef EXIT_UPON_QP_FAIL
      if (!prevQPsuccess) {
        spdlog::error("timestep={:d} msg=\"unable to solve QP for two "
                      "successive iterations; exiting\"",
                      globalIterNum);
        exit(3);
      }
#endif
      spdlog::error(
          "timestep={:d} msg=\"unable to solve QP; resetting active set\"",
          globalIterNum);
  ...
  if (stopped) {
    // IglUtils::writeSparseMatrixToFile(outputFolderPath +
    // "precondMtr_stopped_" + std::to_string(globalIterNum), precondMtr);
    // logFile << "descent step stopped at overallIter" << globalIterNum << "
    // for no prominent energy decrease." << std::endl;
  }
...
template <int dim>
bool isIntersected(const Mesh<dim> &mesh, const SpatialHash<dim> &sh,
                   const Eigen::MatrixXd &V0, const Config &p_animConfig) {
  ...
  for (const auto &obj : p_animConfig.collisionObjects) {
    if (obj->isIntersected(mesh, V0)) {
      logFile << "mesh intersects with analytical collision object"
              << std::endl;
      return true;
    }
  }
  ...
  for (int coI = 0; coI < p_animConfig.meshCollisionObjects.size(); ++coI) {
    if (!p_animConfig.meshCollisionObjects[coI]->checkEdgeTriIntersectionIfAny(
            mesh, sh)) {
...
template <int dim>
bool Optimizer<dim>::lineSearch(double &stepSize, double armijoParam,
                                double lowerBound) {
  timer_step.start(5);
  ...
  if (outputLineSearch) {
    result.saveAsMesh(outputFolderPath + ((dim == 2) ? "lineSearchBase.obj"
                                                     : "lineSearchBase.msh"));
  }
  ...
  if (!solveIP) {
    ...
    computeEnergyVal(
        result, false,
        lastEnergyVal); // TODO: only for updated constraints set and kappa
    timer_step.start(5);
  }
  ...
  bool output = false;
  if (solveIP &&
      animConfig.ccdMethod != ccd::CCDMethod::FLOATING_POINT_ROOT_FINDER) {
    result.saveSurfaceMesh(outputFolderPath + "before" +
                           std::to_string(numOfCCDFail) + ".obj");
    output = true;
  }
#endif
  stepForward(resultV0, result, stepSize);
  if (energyTerms[0]->getNeedElemInvSafeGuard()) {
    while (!result.checkInversion(true)) {
      logFile << "element inversion detected during line search, backtrack!"
              << std::endl;
      stepSize /= 2.0;
      stepForward(resultV0, result, stepSize);
    }
  ...
  if (solveIP) {
    ...
    while (isIntersected(result, sh, resultV0, animConfig)) {
      logFile << "intersection detected during line search, backtrack!"
              << std::endl;
#ifdef OUTPUT_CCD_FAIL
      if (output) {
        result.saveSurfaceMesh(outputFolderPath + "after" +
                               std::to_string(numOfCCDFail) + ".obj");
        ++numOfCCDFail;
        output = false;
      }
...
#define ARMIJO_RULE
  ...
  while ((testingE > lastEnergyVal + stepSize * c1m) && // Armijo condition
    ...
    if (stepSize == 0.0) {
      stopped = true;
      if (!mute) {
        logFile << "testingE" << globalIterNum << " " << testingE << " > "
                << lastEnergyVal << " " << stepSize * c1m << std::endl;
        //                    logFile << "testingG" << globalIterNum << " " <<
        //                    searchDir.dot(testingG) << " < " << c2m <<
        //                    std::endl;
      }
      break;
    }
  ...
  if (stepSize < LFStepSize && solveIP) {
    ...
    while (isIntersected(result, sh, resultV0, animConfig)) {
      logFile << "intersection detected after line search, backtrack!"
              << std::endl;
      stepSize /= 2.0;
      stepForward(resultV0, result, stepSize);
      sh.build(result, result.avgEdgeLen / 3.0);
  ...
  SelfCollisionHandler<dim>::largestFeasibleStepSize_CCD_exact(
      result, sh, searchDir, ccd::CCDMethod::RATIONAL_ROOT_PARITY,
      stepSize_rational);
  if (stepSize_rational != stepSize) {
    std::cout << "rational CCD detects interpenetration but inexact didn't"
              << std::endl;
  }
  ...
  // if (energyTerms[0]->getNeedElemInvSafeGuard()) {
  //     if (!result.checkInversion(true)) {
  //         logFile << "element inversion detected after line search,
  //         backtrack!" << std::endl; stepSize /= 2.0; stepForward(resultV0,
  //         result, stepSize);
  //     }
  // }
  // if (solveIP) {
  //     while (isIntersected(result, sh, resultV0, animConfig)) {
  //         logFile << "intersection detected after line search, backtrack!" <<
  //         std::endl; stepSize /= 2.0; stepForward(resultV0, result,
  //         stepSize); sh.build(result, result.avgEdgeLen / 3.0);
  //     }
  // }
  ...
  if (outputLineSearch) {
    IglUtils::writeVectorToFile(outputFolderPath + "searchDir.txt", searchDir);
    exit(0);
  }
...
template <int dim>
void Optimizer<dim>::stepForward(const Eigen::MatrixXd &dataV0, Mesh<dim> &data,
...
#ifdef USE_TBB
  tbb::parallel_for(0, (int)data.V.rows(), 1,
                    [&](int vI)
#else
  for (int vI = 0; vI < data.V.rows(); vI++)
#endif
                    {
                      data.V.row(vI) =
                          dataV0.row(vI) +
                          stepSize *
                              searchDir.segment<dim>(vI * dim).transpose();
                    }
#ifdef USE_TBB
  );
#endif
...
template <int dim> void Optimizer<dim>::updateTargetGRes(void) {
  targetGRes = std::sqrt(
...
void Optimizer<dim>::saveStatus(const std::string &appendStr) {
  std::string status_fname =
      fmt::format("{}status{:d}{}", outputFolderPath, globalIterNum, appendStr);
...
#ifdef USE_TBB
  tbb::parallel_for(0, (int)V_surf.rows(), 1,
                    [&](int vI)
#else
  for (int vI = 0; vI < V_surf.rows(); ++vI)
#endif
  ...
  igl::writeOBJ(outputFolderPath + std::to_string(globalIterNum) + ".obj",
                V_surf.rowwise() + invShift.transpose(), F_surf);

  // codimensional segment CO
  if (result.CE.rows()) {
#ifdef USE_TBB
    tbb::parallel_for(0, (int)V_CE.rows(), 1,
                      [&](int vI)
#else
...
#endif
    IglUtils::writeSEG(
        outputFolderPath + std::to_string(globalIterNum) + ".seg", V_CE, F_CE);
  }
  ...
  if (hasPointCO) {
    FILE *out =
        fopen((outputFolderPath + "pt" + std::to_string(globalIterNum) + ".obj")
                  .c_str(),
              "w");
...
template <int dim> void Optimizer<dim>::outputCollStats(std::ostream &out) {
  for (int i = 0; i < n_collPairs_sum.size(); ++i) {
    out << "avg # collision pairs / iter = "
        << n_collPairs_sum[i] / (innerIterAmt ? innerIterAmt : 1) << std::endl;
    out << "max # collision pairs / iter = " << n_collPairs_max[i] << std::endl;
    out << "avg # collision pairs / end of ts = "
        << n_convCollPairs_sum[i] / globalIterNum << std::endl;
    out << "max # collision pairs / end of ts = " << n_convCollPairs_max[i]
        << std::endl;
    out << std::endl;
  }
  ...
  out << "Total:" << std::endl;
  out << "avg # collision pairs / iter = "
      << n_collPairs_sum.sum() / (innerIterAmt ? innerIterAmt : 1) << std::endl;
  out << "max # collision pairs / iter = " << n_collPairs_total_max
      << std::endl;
  out << "avg # collision pairs / end of ts = "
      << n_convCollPairs_sum.sum() / globalIterNum << std::endl;
  out << "max # collision pairs / end of ts = " << n_convCollPairs_total_max
      << std::endl;
  out << std::endl;
}
...
void Optimizer<dim>::saveFrictionData(const Mesh<dim> &mesh) const {
  ...
  std::ofstream json(fmt::format("{}friction_data_{:d}.json", outputFolderPath,
                                 output_counter++),
                     std::ios::out);
  ...
  json << "\"closest_point_coordinates\":"
       << coords_matrix.format(matrixJSONFmt) << ",\n";
  ...
  friction_hessian.setFromTriplets(friction_hessian_triplets.begin(),
                                   friction_hessian_triplets.end());
  ...
  for (int k = 0; k < friction_hessian.outerSize(); ++k) {
    for (Eigen::SparseMatrix<double>::InnerIterator it(friction_hessian, k); it;
         ++it) {
      json << triplet_prefix << "[" << it.row() << "," << it.col() << ","
           << it.value() << "]";
      triplet_prefix = ",";
    }
  }
...
template <int dim>
void Optimizer<dim>::computeEnergyVal(const Mesh<dim> &data, int redoSVD,
                                      double &energyVal) {
  ...
  case TIT_BE: {
    ...
    for (int eI = 1; eI < energyTerms.size(); eI++) {
      energyTerms[eI]->computeEnergyVal(
          data, redoSVD, svd, F, dtSq * energyParams[eI], energyVal_ET[eI]);
  ...
  case TIT_NM: {
    energyTerms[0]->computeEnergyVal(data, redoSVD, svd, F,
                                     dtSq * beta_NM * energyParams[0],
                                     energyVal_ET[0]);
    energyVal = energyVal_ET[0];
    for (int eI = 1; eI < energyTerms.size(); eI++) {
      energyTerms[eI]->computeEnergyVal(data, redoSVD, svd, F,
                                        dtSq * beta_NM * energyParams[eI],
                                        energyVal_ET[eI]);
      energyVal += energyVal_ET[eI];
    }
    break;
  ...
  for (int vI = 0; vI < data.V.rows(); vI++)
                    ...
                    {
                      energyVals[vI] =
                          ((data.V.row(vI) - xTilta.row(vI)).squaredNorm() *
    ...
    for (int NBCi = 0; NBCi < data.NeumannBCs.size(); NBCi++) {
      ...
      for (const auto &vI : NBC.vertIds) {
        if (!data.isDBCVertex(vI) && animScripter.isNBCActive(data, NBCi)) {
          energyVal -= dtSq * data.massMatrix.coeff(vI, vI) *
                       data.V.row(vI).dot(NBC.force.transpose());
        }
      }
    }
  }
  ...
  if (solveIP) {
    ...
    for (int coI = 0; coI < animConfig.meshCollisionObjects.size(); ++coI) {
      int startCI = constraintVals.size();
      animConfig.meshCollisionObjects[coI]->evaluateConstraints(
          data, MMActiveSet[coI], constraintVals);
      bVals.conservativeResize(constraintVals.size());
      for (int cI = startCI; cI < constraintVals.size(); ++cI) {
        if (constraintVals[cI] <= 0.0) {
          spdlog::error("constraintVal = {:g} when evaluating constraints",
      ...
      animConfig.meshCollisionObjects[coI]->augmentParaEEEnergy(
          data, paraEEMMCVIDSet[coI], paraEEeIeJSet[coI], constraintVals, bVals,
          dHat, 1.0);
    }
    if (animConfig.isSelfCollision) {
      int startCI = constraintVals.size();
      SelfCollisionHandler<dim>::evaluateConstraints(data, MMActiveSet.back(),
                                                     constraintVals);
      bVals.conservativeResize(constraintVals.size());
      // TODO: parallelize
      for (int cI = startCI; cI < constraintVals.size(); ++cI) {
        if (constraintVals[cI] <= 0.0) {
          spdlog::error("constraintVal = {:g} when evaluating constraints {:d} "
                        "{:d} {:d} {:d}",
                        constraintVals[cI], MMActiveSet.back()[cI][0],
                        MMActiveSet.back()[cI][1], MMActiveSet.back()[cI][2],
                        MMActiveSet.back()[cI][3]);
          std::cout << data.V.row(MMActiveSet.back()[cI][0] >= 0
                                      ? MMActiveSet.back()[cI][0]
                                      : (-MMActiveSet.back()[cI][0] - 1))
                    << std::endl;
          std::cout << data.V.row(MMActiveSet.back()[cI][1] >= 0
                                      ? MMActiveSet.back()[cI][1]
                                      : (-MMActiveSet.back()[cI][1] - 1))
                    << std::endl;
          std::cout << data.V.row(MMActiveSet.back()[cI][2] >= 0
                                      ? MMActiveSet.back()[cI][2]
                                      : (-MMActiveSet.back()[cI][2] - 1))
                    << std::endl;
          std::cout << data.V.row(MMActiveSet.back()[cI][3] >= 0
                                      ? MMActiveSet.back()[cI][3]
                                      : (-MMActiveSet.back()[cI][3] - 1))
                    << std::endl;
      ...
      SelfCollisionHandler<dim>::evaluateConstraints(
          data, paraEEMMCVIDSet.back(), constraintVals);
      bVals.conservativeResize(constraintVals.size());
      for (int cI = startCI; cI < constraintVals.size(); ++cI) {
        if (constraintVals[cI] <= 0.0) {
          spdlog::error("constraintVal = {:g} when evaluating constraints {:d} "
          ...
          if (MMCVIDI[3] >= 0) {
            // EE
            compute_eps_x(data, MMCVIDI[0], MMCVIDI[1], MMCVIDI[2], MMCVIDI[3],
                          eps_x);
            compute_e(data.V.row(MMCVIDI[0]), data.V.row(MMCVIDI[1]),
                      data.V.row(MMCVIDI[2]), data.V.row(MMCVIDI[3]), eps_x, e);
          } else {
            // PP or PE
            const std::pair<int, int> &eIeJ =
                paraEEeIeJSet.back()[cI - startCI];
            const std::pair<int, int> &eI = data.SFEdges[eIeJ.first];
            const std::pair<int, int> &eJ = data.SFEdges[eIeJ.second];
            compute_eps_x(data, eI.first, eI.second, eJ.first, eJ.second,
                          eps_x);
            compute_e(data.V.row(eI.first), data.V.row(eI.second),
                      data.V.row(eJ.first), data.V.row(eJ.second), eps_x, e);
          }
          compute_b(constraintVals[cI], dHat, bVals[cI]);
          bVals[cI] *= e;
        }
    ...
    for (int coI = 0; coI < animConfig.collisionObjects.size(); ++coI) {
      // friction
      if (activeSet_lastH[coI].size() && fricDHat > 0.0 &&
          animConfig.collisionObjects[coI]->friction > 0.0) {
        double Ef;
        animConfig.collisionObjects[coI]->computeFrictionEnergy(
            data.V, result.V_prev, activeSet_lastH[coI], lambda_lastH[coI], Ef,
            fricDHat, 1.0);
        energyVal += Ef;
        // std::cout << ", E_f" << coI << "=" << Ef;
        // energies[2] = Ef;
    ...
    if (animConfig.isSelfCollision) {
      if (MMActiveSet_lastH.back().size() && fricDHat > 0.0 &&
          animConfig.selfFric > 0.0) {
        double Ef;
        SelfCollisionHandler<dim>::computeFrictionEnergy(
            data.V, result.V_prev, MMActiveSet_lastH.back(),
            MMLambda_lastH.back(), MMDistCoord.back(), MMTanBasis.back(), Ef,
            fricDHat, animConfig.selfFric);
        energyVal += Ef;
        // std::cout << ", E_fs=" << Ef;
        // energies[3] = Ef;
...
#endif
        {
          displacement.segment<dim>(vI * dim) =
              (data.V.row(vI) - result.V_prev.row(vI)).transpose();
        }
#ifdef USE_TBB
    );
#endif
...
template <int dim>
void Optimizer<dim>::computeGradient(const Mesh<dim> &data, bool redoSVD,
  ...
  case TIT_NM: {
    energyTerms[0]->computeGradient(data, redoSVD, svd, F,
                                    dtSq * beta_NM * energyParams[0],
                                    gradient_ET[0], projectDBC);
    gradient = gradient_ET[0];
    for (int eI = 1; eI < energyTerms.size(); eI++) {
      energyTerms[eI]->computeGradient(data, redoSVD, svd, F,
                                       dtSq * beta_NM * energyParams[eI],
                                       gradient_ET[eI], projectDBC);
      gradient += gradient_ET[eI];
    }
    break;
  }
  ...
  if (animScripter.isNBCActive()) {
    for (int NBCi = 0; NBCi < data.NeumannBCs.size(); NBCi++) {
      ...
      for (const auto &vI : NBC.vertIds) {
        if (!data.isDBCVertex(vI) && animScripter.isNBCActive(data, NBCi)) {
          gradient.template segment<dim>(vI * dim) -=
              dtSq * data.massMatrix.coeff(vI, vI) * NBC.force;
        }
      }
    }
  }
  ...
  if (solveIP) {
    ...
    for (int coI = 0; coI < animConfig.collisionObjects.size(); ++coI) {
      int startCI = constraintVal.size();
      animConfig.collisionObjects[coI]->evaluateConstraints(
          data, activeSet[coI], constraintVal);
      for (int cI = startCI; cI < constraintVal.size(); ++cI) {
        compute_g_b(constraintVal[cI], dHat, constraintVal[cI]);
      }
      animConfig.collisionObjects[coI]->leftMultiplyConstraintJacobianT(
          data, activeSet[coI],
          constraintVal.segment(startCI, activeSet[coI].size()), gradient,
          kappa);

      // friction
      if (activeSet_lastH[coI].size() && fricDHat > 0.0 &&
          animConfig.collisionObjects[coI]->friction > 0.0) {
        animConfig.collisionObjects[coI]->augmentFrictionGradient(
            data.V, result.V_prev, activeSet_lastH[coI], lambda_lastH[coI],
            gradient, fricDHat, 1.0);
      }
    }
    for (int coI = 0; coI < animConfig.meshCollisionObjects.size(); ++coI) {
      int startCI = constraintVal.size();
      animConfig.meshCollisionObjects[coI]->evaluateConstraints(
          data, MMActiveSet[coI], constraintVal);
      for (int cI = startCI; cI < constraintVal.size(); ++cI) {
        compute_g_b(constraintVal[cI], dHat, constraintVal[cI]);
      }
      animConfig.meshCollisionObjects[coI]->leftMultiplyConstraintJacobianT(
          data, MMActiveSet[coI],
          constraintVal.segment(startCI, MMActiveSet[coI].size()), gradient,
          kappa);

      animConfig.meshCollisionObjects[coI]->augmentParaEEGradient(
          data, paraEEMMCVIDSet[coI], paraEEeIeJSet[coI], gradient, dHat,
          kappa);
    }
    if (animConfig.isSelfCollision) {
      int startCI = constraintVal.size();
      ...
      SelfCollisionHandler<dim>::leftMultiplyConstraintJacobianT(
          data, MMActiveSet.back(),
          constraintVal.segment(startCI, MMActiveSet.back().size()), gradient,
          kappa);

      SelfCollisionHandler<dim>::augmentParaEEGradient(
          data, paraEEMMCVIDSet.back(), paraEEeIeJSet.back(), gradient, dHat,
          kappa);
      ...
      if (MMActiveSet_lastH.back().size() && fricDHat > 0.0 &&
        ...
        SelfCollisionHandler<dim>::augmentFrictionGradient(
            ...
            gradient, fricDHat, animConfig.selfFric);
#ifdef EXPORT_FRICTION_DATA
...
template <int dim>
void Optimizer<dim>::computePrecondMtr(
    const Mesh<dim> &data, bool redoSVD,
  ...
  if (solveIP) {
    ...
    timer_step.start(1);
    if (animConfig.isSelfCollision &&
        (MMActiveSet.back().size() + paraEEeIeJSet.back().size() +
         MMActiveSet_lastH.back().size())) {
      // there is extra connectivity from self-contact
      timer_mt.start(8);
      std::vector<std::set<int>> vNeighbor_IP_new = data.vNeighbor;
      SelfCollisionHandler<dim>::augmentConnectivity(data, MMActiveSet.back(),
                                                     vNeighbor_IP_new);
      SelfCollisionHandler<dim>::augmentConnectivity(
          data, paraEEMMCVIDSet.back(), paraEEeIeJSet.back(), vNeighbor_IP_new);
      if (MMActiveSet_lastH.back().size() && fricDHat > 0.0 &&
          animConfig.selfFric > 0.0) {
        SelfCollisionHandler<dim>::augmentConnectivity(
            data, MMActiveSet_lastH.back(), vNeighbor_IP_new);
      }
      timer_mt.stop();

      if (vNeighbor_IP_new != vNeighbor_IP || animConfig.doAnalysis) {
        timer_mt.start(8);
    ...
    } else if (vNeighbor_IP != data.vNeighbor || animConfig.doAnalysis) {
  ...
  if (updateDamping && animConfig.dampingStiff) {
    ...
    case TIT_NM: {
      p_linSysSolver->setCoeff(dampingMtr, 1.0 + dtSq * beta_NM * dt /
                                                     animConfig.dampingStiff);
      break;
    }
    }
    //        linSysSolver->saveHessian("0"); // HessDamping
  } else {
    ...
    case TIT_BE: {
      for (int eI = 0; eI < energyTerms.size(); eI++) {
        energyTerms[eI]->computeHessian(data, redoSVD, svd, F,
                                        energyParams[eI] * dtSq, p_linSysSolver,
                                        true, projectDBC);
      }
      break;
    }
    ...
    //        linSysSolver->saveHessian("1"); // HessNoDamping
  }
  ...
  //    linSysSolver->saveHessian("2"); // HessDirichlet
  if (solveIP) {
    timer_mt.start(12);
    for (int coI = 0; coI < animConfig.collisionObjects.size(); ++coI) {
      animConfig.collisionObjects[coI]->augmentIPHessian(
          data, activeSet[coI], p_linSysSolver, dHat, kappa, projectDBC);

      // friction
      if (activeSet_lastH[coI].size() && fricDHat > 0.0 &&
          animConfig.collisionObjects[coI]->friction > 0.0) {
        animConfig.collisionObjects[coI]->augmentFrictionHessian(
            data, result.V_prev, activeSet_lastH[coI], lambda_lastH[coI],
            p_linSysSolver, fricDHat, 1.0, projectDBC);
      }
    }
    timer_mt.start(13);
    for (int coI = 0; coI < animConfig.meshCollisionObjects.size(); ++coI) {
      animConfig.meshCollisionObjects[coI]->augmentIPHessian(
          data, MMActiveSet[coI], p_linSysSolver, dHat, kappa, projectDBC);

      animConfig.meshCollisionObjects[coI]->augmentParaEEHessian(
          data, paraEEMMCVIDSet[coI], paraEEeIeJSet[coI], p_linSysSolver, dHat,
          kappa, projectDBC);
    }
    timer_mt.start(14);
    if (animConfig.isSelfCollision) {
      SelfCollisionHandler<dim>::augmentIPHessian(
          data, MMActiveSet.back(), p_linSysSolver, dHat, kappa, projectDBC);

      SelfCollisionHandler<dim>::augmentParaEEHessian(
          data, paraEEMMCVIDSet.back(), paraEEeIeJSet.back(), p_linSysSolver,
          dHat, kappa, projectDBC);

      if (MMActiveSet_lastH.back().size() && fricDHat > 0.0 &&
          animConfig.selfFric > 0.0) {
        SelfCollisionHandler<dim>::augmentFrictionHessian(
            data, result.V_prev, MMActiveSet_lastH.back(),
            MMLambda_lastH.back(), MMDistCoord.back(), MMTanBasis.back(),
            p_linSysSolver, fricDHat, animConfig.selfFric, projectDBC);
      }
    }
    timer_mt.stop();
    //        linSysSolver->saveHessian("3"); // HessIP
  }

  if (animConfig.dampingStiff && (!updateDamping)) {
    p_linSysSolver->addCoeff(dampingMtr, 1.0);
    //        linSysSolver->saveHessian("4"); // HessSecondDamping
  }

  if (!projectDBC && rho_DBC) {
    animScripter.augmentMDBCHessian(data, p_linSysSolver, rho_DBC);
    //        linSysSolver->saveHessian("5"); // HessDBS
  }
  ...
  // IglUtils::writeSparseMatrixToFile("/Users/mincli/Desktop/IPC/output/A",
  // p_linSysSolver, true); std::cout << "matrix written" << std::endl; exit(0);
}

template <int dim>
void Optimizer<dim>::computeDampingMtr(
    const Mesh<dim> &data, bool redoSVD,
    LinSysSolver<Eigen::VectorXi, Eigen::VectorXd> *p_dampingMtr,
    bool projectDBC) {
  p_dampingMtr->setZero();
  for (int eI = 0; eI < energyTerms.size(); eI++) {
    energyTerms[eI]->computeHessian(
        data, redoSVD, svd, F, energyParams[eI] * animConfig.dampingStiff / dt,
        p_dampingMtr, true, projectDBC);
  }
}

template <int dim>
void Optimizer<dim>::setupDampingMtr(
    const Mesh<dim> &data, bool redoSVD,
    LinSysSolver<Eigen::VectorXi, Eigen::VectorXd> *p_dampingMtr) {
...
template <int dim>
void Optimizer<dim>::computeSystemEnergy(
    std::vector<double> &sysE, std::vector<Eigen::Matrix<double, 1, dim>> &sysM,
    std::vector<Eigen::Matrix<double, 1, dim>> &sysL) {
  Eigen::VectorXd energyValPerElem;
  energyTerms[0]->getEnergyValPerElemBySVD(result, true, svd, F,
                                           energyValPerElem, false);
  ...
  for (int compI = 0; compI < compVAccSize.size(); ++compI) {
    ...
    for (int fI = (compI ? compFAccSize[compI - 1] : 0);
         fI < compFAccSize[compI]; ++fI) {
      sysE[compI] += energyValPerElem[fI];
    }
    for (int vI = (compI ? compVAccSize[compI - 1] : 0);
         vI < compVAccSize[compI]; ++vI) {
      sysE[compI] += result.massMatrix.coeff(vI, vI) *
                     ((result.V.row(vI) - result.V_prev.row(vI)).squaredNorm() /
                          dtSq / 2.0 -
                      gravity.dot(result.V.row(vI).transpose()));

      Eigen::Matrix<double, 1, dim> p =
          result.massMatrix.coeff(vI, vI) / dt *
          (result.V.row(vI) - result.V_prev.row(vI));
      sysM[compI] += p;
...
template <int dim> void Optimizer<dim>::checkGradient(void) {
  ...
  for (int vI = 0; vI < result.V.rows(); vI++) {
    for (int dimI = 0; dimI < dim; dimI++) {
      ...
      gradient_finiteDiff[vI * dim + dimI] =
          (energyVal_perturbed - energyVal0) / h;
    }
...
template <int dim> void Optimizer<dim>::checkHessian(void) {
  ...
  for (int vI = 0; vI < result.V.rows(); vI++) {
    ...
    for (int dimI = 0; dimI < dim; dimI++) {
      ...
      // computeConstraintSets(perturbed); // the transition is only C1
      // continuous
      ...
      for (int rowI = 0; rowI < result.V.rows() * dim; rowI++) {
        if (result.DBCVertexIds.find(rowI / dim) == result.DBCVertexIds.end() &&
  ...
  LinSysSolver<Eigen::VectorXi, Eigen::VectorXd> *linSysSolver =
      LinSysSolver<Eigen::VectorXi, Eigen::VectorXd>::create(
          animConfig.linSysSolverType, std::vector<double>());
  linSysSolver->set_pattern(result.vNeighbor, result.DBCVertexIds);
  computeConstraintSets(result);
  computePrecondMtr(result, true, linSysSolver);
  linSysSolver->getCoeffMtr(hessian_symbolicPK);

  Eigen::SparseMatrix<double> difMtrPK =
      hessian_symbolicPK - hessian_finiteDiff;
  ...
  IglUtils::writeSparseMatrixToFile(outputFolderPath + "H_symbolicPK",
                                    hessian_symbolicPK, true);

  IglUtils::writeSparseMatrixToFile(outputFolderPath + "H_finiteDiff",
                                    hessian_finiteDiff, true);
}
