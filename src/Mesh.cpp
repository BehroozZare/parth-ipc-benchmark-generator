...
template <int dim> Mesh<dim>::Mesh(void) {}

template <int dim>
Mesh<dim>::Mesh(
    const Eigen::MatrixXd &V_mesh, const Eigen::MatrixXi &F_mesh,
    const Eigen::MatrixXi &SF_mesh, const Eigen::MatrixXi &CE_mesh,
    ...
    const std::vector<std::pair<Eigen::Vector3i, Eigen::Vector3d>>
        &p_componentMaterial,
    const std::vector<std::pair<Eigen::Vector3i, Eigen::Vector3d>>
        &p_componentLVels,
    const std::vector<std::pair<Eigen::Vector3i, Eigen::Vector3d>>
        &p_componentAVels,
    const std::vector<
        std::pair<Eigen::Vector3i, std::array<Eigen::Vector3d, 2>>>
        &p_componentInitVels,
...
void Mesh<dim>::computeMassMatrix(const igl::MassMatrixType type) {
  ...
  if (simplex_size == 3) {
    ...
    case MASSMATRIX_TYPE_VORONOI: {
      ...
      barycentric =
          (barycentric.array().colwise() / barycentric.array().rowwise().sum())
              .eval();
  ...
  if constexpr (dim == 3) {
    ...
    for (int compI = 0; compI < componentCoDim.size(); ++compI) {
      if (componentCoDim[compI] == 3) {
        ...
        MI.block(oldRows, 0, m, 1) =
            CE.col(0).segment(componentCERange[compI], m);
        MI.block(oldRows + m, 0, m, 1) =
            CE.col(1).segment(componentCERange[compI], m);
        ...
        for (int i = 0; i < m; ++i) {
          double l = (V.row(CE(componentCERange[compI] + i, 0)) -
                      V.row(CE(componentCERange[compI] + i, 1)))
                         .norm();
        ...
        MI.bottomRows(m) = Eigen::VectorXi::LinSpaced(
            m, componentCERange[compI], componentCERange[compI + 1] - 1);
        ...
        MI.block(oldRows, 0, m, 1) =
            SF.col(0).segment(componentSFRange[compI], m);
        MI.block(oldRows + m, 0, m, 1) =
            SF.col(1).segment(componentSFRange[compI], m);
        MI.block(oldRows + m * 2, 0, m, 1) =
            SF.col(2).segment(componentSFRange[compI], m);
        ...
        // loop over faces
        for (int i = 0; i < m; i++) {
          l(i, 0) = (V.row(SF(componentSFRange[compI] + i, 1)) -
                     V.row(SF(componentSFRange[compI] + i, 2)))
                        .norm();
          l(i, 1) = (V.row(SF(componentSFRange[compI] + i, 2)) -
                     V.row(SF(componentSFRange[compI] + i, 0)))
                        .norm();
          l(i, 2) = (V.row(SF(componentSFRange[compI] + i, 0)) -
                     V.row(SF(componentSFRange[compI] + i, 1)))
                        .norm();
        }
        Matrix<Scalar, Dynamic, 1> dblA;
        doublearea(l, 0., dblA);
        ...
        Eigen::VectorXd avgELen =
            l.rowwise().mean() /
            3.0; // multiply it on pi*r^2 to approximate 2/3*pi*r^3
        MV.block(oldRows + 0 * m, 0, m, 1) =
            MV.block(oldRows + 0 * m, 0, m, 1).cwiseProduct(avgELen);
        MV.block(oldRows + 1 * m, 0, m, 1) =
            MV.block(oldRows + 1 * m, 0, m, 1).cwiseProduct(avgELen);
        MV.block(oldRows + 2 * m, 0, m, 1) =
            MV.block(oldRows + 2 * m, 0, m, 1).cwiseProduct(avgELen);
      }
    }
  }
  ...
  for (int compI = 0; compI < componentCoDim.size(); ++compI) {
    if (componentCoDim[compI] == 0) {
      for (int vI = componentNodeRange[compI];
           vI < componentNodeRange[compI + 1]; ++vI) {
...
void Mesh<dim>::computeFeatures(bool multiComp, bool resetDBCV) {
  ...
  SFEdges =
      std::vector<std::pair<int, int>>(SFEdges_set.begin(), SFEdges_set.end());
  for (int ceI = 0; ceI < CE.rows(); ++ceI) {
...
template <int dim>
void Mesh<dim>::resetDBCVertices(
    const std::map<int, DirichletBCType> &p_DBCVert) {
  DBCVertexIds.clear();
  vertexDBCType.clear();
  vertexDBCType.resize(V.rows(), DirichletBCType::NOT_DBC);
...
template <int dim> void Mesh<dim>::addDBCVertex(int vI, DirichletBCType type) {
  assert(vI < V.rows());
  vertexDBCType[vI] = type;
  if (type == DirichletBCType::NOT_DBC) {
...
template <int dim>
void Mesh<dim>::addDBCVertices(
    const std::vector<std::pair<int, DirichletBCType>> &p_DBCVert) {
  for (const auto &[vI, type] : p_DBCVert) {
...
template <int dim>
void Mesh<dim>::addDBCVertices(const std::vector<int> &p_DBCVert,
...
template <int dim> void Mesh<dim>::removeDBCVertex(int vI) {
  assert(vI < V.rows());
  DBCVertexIds.erase(vI);
  vertexDBCType[vI] = DirichletBCType::NOT_DBC;
}

template <int dim> double Mesh<dim>::avgNodeMass(int coDim) const {
  ...
  if (coDim == -1) {
    ...
    for (int compI = 0; compI < componentCoDim.size(); ++compI) {
      if (componentCoDim[compI] == coDim) {
        mSum += massMatrix.diagonal()
                    .segment(componentNodeRange[compI],
                             componentNodeRange[compI + 1] -
                                 componentNodeRange[compI])
                    .sum();
...
template <int dim> double Mesh<dim>::matSpaceBBoxSize2(int coDim) const {
  if (coDim == -1) {
    // include all nodes
    return (V_rest.colwise().maxCoeff() - V_rest.colwise().minCoeff())
        .squaredNorm();
  } else {
    assert(coDim >= 0 && coDim <= 3);
    ...
    for (int compI = 0; compI < componentCoDim.size(); ++compI) {
      if (componentCoDim[compI] == coDim) {
        bottomLeft = bottomLeft.min(V_rest
                                        .block(componentNodeRange[compI], 0,
                                               componentNodeRange[compI + 1] -
                                                   componentNodeRange[compI],
                                               dim)
                                        .colwise()
...
template <int dim> int Mesh<dim>::vICoDim(int vI) const {
  int compI = 1;
...
template <int dim> int Mesh<dim>::sfICoDim(int sfI) const {
  int compI = 1;
...
template <int dim> void Mesh<dim>::setLameParam(double YM, double PR) {
  u = Eigen::VectorXd::Constant(F.rows(), YM / 2.0 / (1.0 + PR));
  lambda = Eigen::VectorXd::Constant(F.rows(),
                                     YM * PR / (1.0 + PR) / (1.0 - 2.0 * PR));
  for (const auto &matI : componentMaterial) {
    massMatrix.diagonal().segment(componentNodeRange[matI.first[0]],
                                  componentNodeRange[matI.first[0] + 1] -
                                      componentNodeRange[matI.first[0]]) *=
        matI.second[0] / density;
    u.segment(matI.first[1], matI.first[2] - matI.first[1])
        .setConstant(matI.second[1] / 2.0 / (1.0 + matI.second[2]));
    lambda.segment(matI.first[1], matI.first[2] - matI.first[1])
        .setConstant(matI.second[1] * matI.second[2] / (1.0 + matI.second[2]) /
                     (1.0 - 2.0 * matI.second[2]));
  }
  ...
  //            center /= dim + 1;
  //
  //            int segI = std::min(nSeg - 1, std::max(0, int((center[0] - xMin)
  //            / xSegLen))); if(segI % 2 == 0) {
  //                u[elemI] = u_stiffer;
  ...
  //        double yThres = (V_rest.col(1).maxCoeff() +
  //        V_rest.col(1).minCoeff()) / 2.0; YM *= 10; double u_stiffer = YM
  //        / 2.0 / (1.0 + PR); double lambda_stiffer = YM * PR / (1.0 + PR) /
  //        (1.0 - 2.0 * PR); for(int elemI = 0; elemI < F.rows(); ++elemI) {
  //            const Eigen::Matrix<int, 1, dim + 1>& elemVInd = F.row(elemI);
  //            Eigen::RowVector3d center;
  //            center.setZero();
  //            for(int i = 0; i < dim + 1; ++i) {
...
template <int dim> bool Mesh<dim>::checkInversion(int triI, bool mute) const {
  assert(triI < F.rows());
  ...
  if (dbArea < eps) {
    if (!mute) {
      ...
      logFile << "***Element inversion detected: " << dbArea << " < " << eps
              << std::endl;
    }
    return false;
  } else {
...
template <int dim>
bool Mesh<dim>::checkInversion(bool mute,
...
template <int dim>
void Mesh<dim>::save(const std::string &filePath, const Eigen::MatrixXd &V,
  ...
  if (FUV.rows() == F.rows()) {
    for (int triI = 0; triI < F.rows(); triI++) {
      ...
      out << "f " << tri[0] + 1 << "/" << tri_UV[0] + 1 << " " << tri[1] + 1
          << "/" << tri_UV[1] + 1 << " " << tri[2] + 1 << "/" << tri_UV[2] + 1
          << std::endl;
    }
  } else {
    for (int triI = 0; triI < F.rows(); triI++) {
      ...
      out << "f " << tri[0] + 1 << "/" << tri[0] + 1 << " " << tri[1] + 1 << "/"
          << tri[1] + 1 << " " << tri[2] + 1 << "/" << tri[2] + 1 << std::endl;
    }
  }
...
template <int dim> void Mesh<dim>::save(const std::string &filePath) const {
...
template <int dim>
void Mesh<dim>::saveSurfaceMesh(const std::string &filePath, bool use_V_prev,
                                bool useInvShift) const {
  if constexpr (dim == 3) {
...
template <int dim> bool Mesh<dim>::isBoundaryVert(int vI) const {
