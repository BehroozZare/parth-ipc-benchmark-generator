...
// std::string outputFolderPath = "/media/LargeHard/IPC/output/tests/";
// std::string csvOutputPath =
// "/home/behrooz/Desktop/IPC_Project/IPC-dev2/output/";
//  std::string csvOutputPath = "/u/behrooz/Graphics/IPC-dev2/output";
// std::string outputFolderPath =
// "/home/behrooz/Desktop/IPC_Project/IPC-dev2/output/"; std::string
// outputFolderPath = "/media/LargeHard/IPC/test_out/";
std::string csvOutputPath = "output/";
...
int showDistortion =
    2; // 0: don't show; 1: energy value; 2: other scalar field;
...
void saveScreenshot(const std::string &filePath, double scale = 1.0,
                    bool writeGIF = false, bool writePNG = true);
void saveInfoForPresent(const std::string fileName = "info.txt",
                        double save_dt = 1e-2);

bool useProgressBar = true;
void printProgressBar(int cur_iter, int max_iter, int bar_width = 70,
  ...
  fmt::print("{3:2d}%|{0:█>{1}}{0: >{2}}| {4: >{6}d}/{5:d}\r", //
             "", pos, std::max(bar_width - pos, 0), int(progress * 100.0),
             cur_iter, max_iter, len);
  if (progress >= 1) {
    if (clear_end) {
...
void proceedOptimization(int proceedNum = 1) {
  for (int proceedI = 0; (proceedI < proceedNum) && (!converged); proceedI++) {
    ...
    if (!offlineMode) {
      ...
      saveInfo(/*writePNG=*/false, /*writeGIF=*/true,
               /*writeMesh=*/false); // save gif
      if (savePNG) {
        saveScreenshot(outputFolderPath + infoName + ".png", 1.0, false, true);
      }
    }
    ...
    if (useProgressBar && !converged &&
        spdlog::get_level() >= spdlog::level::level_enum::warn) {
...
void updateViewerData_distortion(void) {
  ...
  case 2: { // show other triangle-based scalar fields
    ...
    faceWeight *= -1.0;
#endif
    igl::colormap(igl::COLOR_MAP_TYPE_VIRIDIS, faceWeight, true,
                  color_distortionVis);
...
#if (DIM == 2)
    color_distortionVis =
        Eigen::MatrixXd::Ones(triSoup[viewChannel]->F.rows(), 3);
#else
  ...
  case -1: {
    assert(faceColors_default.rows() ==
           ((DIM == 3) ? SF.rows() : triSoup[viewChannel]->F.rows()));
  ...
  if (floorColor.rows() > 0) {
    color_distortionVis.conservativeResize(
        color_distortionVis.rows() + floorColor.rows(), 3);
...
void updateViewerData(void) {
  ...
  if (viewUV) {
    ...
    viewer.core().align_camera_center((config.cameraTracking
                                           ? triSoup[viewChannel]->V
                                           : triSoup[viewChannel]->V_rest),
                                      F_vis);
    ...
    viewer.data().set_points(Eigen::MatrixXd::Zero(0, 3),
                             Eigen::RowVector3d::Zero());
    for (const auto &vi : triSoup[viewChannel]->DBCVertexIds) {
      if (showZeroDirichletVerts && triSoup[viewChannel]->vertexDBCType[vi] ==
                                        IPC::DirichletBCType::ZERO) {
        viewer.data().add_points(UV_vis.row(vi), zeroDirichletVertsColor);
      } else if (showNonzeroDirichletVerts &&
                 triSoup[viewChannel]->vertexDBCType[vi] ==
                     IPC::DirichletBCType::NONZERO) {
    ...
    if (showNeumannVerts && optimizer->getAnimScripter().isNBCActive()) {
      ...
      for (const auto &NBC : triSoup[viewChannel]->NeumannBCs) {
        if (optimizer->getAnimScripter().isNBCActive(*(triSoup[viewChannel]),
                                                     NBCi)) {
    ...
    for (int ceI = 0; ceI < triSoup[viewChannel]->CE.rows(); ++ceI) {
      viewer.data().add_edges(
          triSoup[viewChannel]->V.row(triSoup[viewChannel]->CE(ceI, 0)),
...
void saveScreenshot(const std::string &filePath, double scale, bool writeGIF,
                    bool writePNG) {
#ifdef USE_OPENGL
  if (offlineMode) {
    return;
  ...
  int width = static_cast<int>(
      scale * (viewer.core().viewport[2] - viewer.core().viewport[0]));
  int height = static_cast<int>(
      scale * (viewer.core().viewport[3] - viewer.core().viewport[1]));
...
void saveInfo(bool writePNG, bool writeGIF, int writeMesh, double save_dt) {
  if (config.playBackSpeed < 1.0) {
    save_dt = std::max(
        config.dt, std::min(save_dt, config.playBackSpeed * GIFDelay / 100.0));
  }
  ...
  if (iterNum != previousSaveIter &&
      iterNum - previousSaveIter < itersBetweenSaves && !converged) {
    return;
  }
  ...
  switch (writeMesh) {
  case 1:
    // triSoup[channel_result]->save(outputFolderPath + infoName +
    // "_triSoup.obj");
...
void saveInfoForPresent(const std::string fileName, double save_dt) {
  ...
  file << vertAmt_input << " " << triSoup[channel_initial]->F.rows()
       << std::endl;

  file << iterNum << " " << optimizer->getInnerIterAmt() << " "
       << double(optimizer->getInnerIterAmt()) / iterNum << std::endl;

  file << "current " << getCurrentRSS() / 1024.0 << " KB, peak "
       << getPeakRSS() / 1024.0 << " KB" << std::endl;
...
void toggleOptimization(void) {
  ...
  if (optimization_on) {
    if (converged) {
      ...
      if (iterNum == 0) {
        GifBegin(
            &GIFWriter, (outputFolderPath + "anim.gif").c_str(),
            GIFScale * (viewer.core().viewport[2] - viewer.core().viewport[0]),
            GIFScale * (viewer.core().viewport[3] - viewer.core().viewport[1]),
            GIFDelay);
...
bool key_pressed(igl::opengl::glfw::Viewer &viewer, unsigned char key,
                 int modifier) {
  switch (key) {
  case '=':
  case '+': {
  ...
  case '-':
  case '_': {
    viewer.data().point_size = std::max(
        1.0f, viewer.data().point_size - (modifier & IGL_MOD_SHIFT ? 10 : 1));
    break;
  }
...
#endif
{
  ...
  if (saveInfo_postDraw) {
    ...
    saveInfo(/*writePNG=*/outerLoopFinished, /*writeGIF=*/true,
             /*writeMesh=*/outerLoopFinished);
    // Note that the content saved in the screenshots are depends on where
    // updateViewerData() is called
    if (outerLoopFinished) {
      //            triSoup[channel_result]->saveAsMesh(outputFolderPath +
      //            infoName + "_mesh_01UV.obj", true);
    }
  }
...
#endif
{
  ...
  viewer.core().is_animating = false;
#endif
  spdlog::info("optimization converged, with {:d} inner iterations in {:g}s.",
               optimizer->getInnerIterAmt(), secPast);
...
struct CLIArgs {
  int progMode = 10;
  const std::vector<std::pair<std::string, int>> PROG_MODE_NAMES_TO_PROG_MODE =
      {
          {"gui", 0},
          {"gui_play", 10},
  ...
  CLIArgs(int argc, char *argv[]) {
    ...
    app.add_option("progMode", progMode, "program mode")
        ->required()
        ->transform(CLI::CheckedTransformer(PROG_MODE_NAMES_TO_PROG_MODE,
                                            CLI::ignore_case))
    ...
    app.add_option("--log,--logLevel", logLevel, "log level")
        ->transform(CLI::CheckedTransformer(SPDLOG_LEVEL_NAMES_TO_LEVELS,
                                            CLI::ignore_case))
    ...
    app.add_option("--CompType", compType,
                   "Choose one of the compression type BLR, HSS, NONE");
    app.add_option("--SimName", simName, "Simulation name");

    app.add_option("--OrderingAlg", ordMethod, "Ordering Name");
    app.add_option("--DoAnalysis", doAnalysis,
                   "Apply symbolic factorization per iteration");

    app.add_option("--SepSize", sepSize,
                   "minimum separation size of strumpack");
    app.add_option("--FrontSize", frontSize, "minimum front size of strumpack");
    app.add_option("--CompRelTol", comp_rel_tol,
                   "minimum Rel tol for COMPRESSION");
    app.add_option("--CompAbsTol", comp_abs_tol,
                   "minimum Abs tol for COMPRESSION");
    app.add_option("--IM", immobilizer, "ADD PARTH IMMOBILIZER");
    app.add_option("--Res", res, "minimum Abs tol for COMPRESSION");
...
int main(int argc, char *argv[]) {
  // initialize colors
  igl::colormap(igl::COLOR_MAP_TYPE_VIRIDIS, 2.0 / 9.0,
                zeroDirichletVertsColor.data());
  igl::colormap(igl::COLOR_MAP_TYPE_VIRIDIS, 4.0 / 9.0,
                nonzeroDirichletVertsColor.data());
  igl::colormap(igl::COLOR_MAP_TYPE_VIRIDIS, 2.0 / 3.0,
                neumannVertsColor.data());

  CLIArgs args(argc, argv);

  // Custom input init
  config.doAnalysis = args.doAnalysis;
  config.sim_name = args.simName;
  config.nthreads = args.numThreads;
  config.strum_par.resize(9, 0);
  if (args.immobilizer == 1) {
    config.strum_par[CONFIG_PAR_PARTH] = 1;
  } else {
    config.strum_par[CONFIG_PAR_PARTH] = 0;
  }

  if (args.ordMethod == "METIS") {
    config.strum_par[CONFIG_PAR_ORDER] = 0;
  } else if (args.ordMethod == "AMD") {
    config.strum_par[CONFIG_PAR_ORDER] = 1;
  } else {
    config.strum_par[CONFIG_PAR_ORDER] = -1;
    std::cerr << "UNKNOWN ORDERING METHOD" << std::endl;
  }

  if (args.res == 1) {
    config.strum_par[CONFIG_PAR_RES] = 1e-1;
  } else if (args.res == 2) {
    config.strum_par[CONFIG_PAR_RES] = 1e-2;
  } else if (args.res == 3) {
    config.strum_par[CONFIG_PAR_RES] = 1e-3;
  } else if (args.res == 4) {
    config.strum_par[CONFIG_PAR_RES] = 1e-4;
  } else if (args.res == 5) {
    config.strum_par[CONFIG_PAR_RES] = 1e-5;
  } else if (args.res == 6) {
    config.strum_par[CONFIG_PAR_RES] = 1e-6;
  } else if (args.res == 7) {
    config.strum_par[CONFIG_PAR_RES] = 1e-7;
  } else if (args.res == 8) {
    config.strum_par[CONFIG_PAR_RES] = 1e-8;
  } else if (args.res == 9) {
    config.strum_par[CONFIG_PAR_RES] = 1e-9;
  }

  if (args.solveLib == "CHOLMOD") {
    config.linSysSolverType = IPC::LinSysSolverType::CHOLMOD;
  } else if (args.solveLib == "EIGEN") {
    config.linSysSolverType = IPC::LinSysSolverType::EIGEN;
  } else if (args.solveLib == "CG") {
    config.linSysSolverType = IPC::LinSysSolverType::CG;
  } else if (args.solveLib == "STRUMPACK") {
    config.linSysSolverType = IPC::LinSysSolverType::STRUMPACK;
    config.Compression = args.compType;
    //    config.strum_par.clear();
    //
    //    if (args.rel_tol == 0) { // rel_tol - 0
    //      config.strum_par.emplace_back(1e-8);
    //    } else {
    //      config.strum_par.emplace_back(1e-8 * args.rel_tol);
    //    }
    //
    //    if (args.abs_tol == 0) { // abs_tol - 1
    //      config.strum_par.emplace_back(1e-10);
    //    } else {
    //      config.strum_par.emplace_back(1e-10 * args.abs_tol);
    //    }
    //
    //    config.strum_par.emplace_back(0); // set_leaf_size - 2
    //    config.strum_par.emplace_back(0); // clustering algorithm - 3

    //    if (args.sepSize == 0) { // min separation size - 5
    //      config.strum_par.emplace_back(args.sepSize);
    //    } else {
    //      config.strum_par.emplace_back(1.0 / args.sepSize);
    //    }
    //
    //    if (args.frontSize == 0) { // min front size - 6
    //      config.strum_par.emplace_back(args.frontSize);
    //    } else {
    //      config.strum_par.emplace_back(1.0 / args.frontSize);
    //    }
    //
    //    if (args.comp_rel_tol == 0) { // min front size - 7
    //      config.strum_par.emplace_back(0);
    //    } else {
    //      config.strum_par.emplace_back(args.comp_rel_tol);
    //    }
    //
    //    if (args.comp_abs_tol == 0) { // min front size - 8
    //      config.strum_par.emplace_back(0);
    //    } else {
    //      config.strum_par.emplace_back(args.comp_abs_tol);
    //    }
  } else if (args.solveLib == "MKL") {
    config.linSysSolverType = IPC::LinSysSolverType::MKL;
  } else if (args.solveLib == "AMGCL") {
    config.linSysSolverType = IPC::LinSysSolverType::AMGCL;
  } else {
    std::cerr << "The solver " << args.solveLib << " is not defined."
              << std::endl;
    return 0;
  }
...
#ifdef IPC_WITH_MKL
  mkl_set_num_threads(args.numThreads);
#endif
  ...
  std::vector<int> componentNodeRange, componentSFRange, componentCERange,
      componentCoDim;
  std::vector<std::pair<Eigen::Vector3i, Eigen::Vector3d>> componentMaterial,
      componentLVels, componentAVels;
    ...
    if (loadSucceed) {
      ...
      for (int i = 0; i < (int)config.inputShapePaths.size(); ++i) {
        ...
        if (!inputShapePath.has_extension()) {
          ...
          const std::string meshFileSuffix =
              inputShapePath.extension().string();
          const std::string inputShapePathNoSuffix =
              inputShapePath.parent_path() / inputShapePath.stem();
          if (meshFileSuffix == ".msh") {
            if (!IPC::IglUtils::readTetMesh(inputShapePathStr, newV, newF,
                                            newSF)) {
              spdlog::error("Unable to read input msh file: {:s}",
                            inputShapePathStr);
              exit(1);
            }
            newE.resize(0, 2);

            if (std::isfinite(config.inputShapeMaterials[i][0]) &&
                std::isfinite(config.inputShapeMaterials[i][1]) &&
                std::isfinite(config.inputShapeMaterials[i][2])) {
              if (config.inputShapeMaterials[i][0] > 0 &&
                  config.inputShapeMaterials[i][1] > 0) {
                Eigen::Vector3i startToEnd;
                startToEnd[0] = i;
                startToEnd[1] = F.rows();
                startToEnd[2] = F.rows() + newF.rows();
                componentMaterial.emplace_back(startToEnd,
                                               config.inputShapeMaterials[i]);
              } else {
                spdlog::warn("Invalid material parameters provided for {} "
                             "(density={:g}, Young's modulus={:g}, and "
                             "Poisson's ratio={:g})), using default material.",
                             inputShapePathStr,
                             config.inputShapeMaterials[i][0],
                             config.inputShapeMaterials[i][1],
                             config.inputShapeMaterials[i][2]);
              }
            }
          } else if (meshFileSuffix == ".ele") {
            IPC::IglUtils::readNodeEle(inputShapePathNoSuffix, newV, newF,
                                       newSF);
            newE.resize(0, 2);
            ...
            if (!igl::readOBJ(inputShapePathStr, newV, newSF)) {
              spdlog::error("Unable to read input obj file: {:s}",
                            inputShapePathStr);
              exit(-1);
            }
            newF.resize(0, 4);
            ...
            if (!IPC::IglUtils::readSEG(inputShapePathStr, newV, newE)) {
              ...
              if (!igl::readOBJ(inputShapePathNoSuffix + ".obj", newV, tempF)) {
                spdlog::error("Unable to read input seg or obj file: {:s}",
                              inputShapePathStr);
                exit(1);
              }
              ...
              for (int sfI = 0; sfI < tempF.rows(); ++sfI) {
                auto finder = edgesSet.find(
                    std::pair<int, int>(tempF(sfI, 1), tempF(sfI, 0)));
                if (finder == edgesSet.end()) {
                  edgesSet.insert(
                      std::pair<int, int>(tempF(sfI, 0), tempF(sfI, 1)));
                }

                finder = edgesSet.find(
                    std::pair<int, int>(tempF(sfI, 2), tempF(sfI, 1)));
                if (finder == edgesSet.end()) {
                  edgesSet.insert(
                      std::pair<int, int>(tempF(sfI, 1), tempF(sfI, 2)));
                }

                finder = edgesSet.find(
                    std::pair<int, int>(tempF(sfI, 0), tempF(sfI, 2)));
                if (finder == edgesSet.end()) {
                  edgesSet.insert(
                      std::pair<int, int>(tempF(sfI, 2), tempF(sfI, 0)));
                }
              }
            ...
            if (!igl::readOBJ(inputShapePathStr, newV, temp)) {
              if (!igl::readOBJ(inputShapePathNoSuffix + ".obj", newV, temp)) {
                spdlog::error("Unable to read input pt or obj file: {:s}",
                              inputShapePathStr);
                exit(1);
              }
            }
            ...
            spdlog::error("Unsupported tet mesh file format: {:s}",
                          meshFileSuffix);
            exit(1);
          }

          if (!std::isnan(config.inputShapeLVels[i][0]) &&
              !std::isnan(config.inputShapeLVels[i][1]) &&
              !std::isnan(config.inputShapeLVels[i][2])) {
            Eigen::Vector3i startToEnd;
            startToEnd[0] = i;
            startToEnd[1] = V.rows();
          ...
          if (!std::isnan(config.inputShapeAVels[i][0]) &&
              !std::isnan(config.inputShapeAVels[i][1]) &&
              !std::isnan(config.inputShapeAVels[i][2])) {
            Eigen::Vector3i startToEnd;
            startToEnd[0] = i;
            startToEnd[1] = V.rows();
            startToEnd[2] = V.rows() + newV.rows();
          ...
          if (!std::isnan(config.inputShapeInitVels[i][0][0]) &&
              !std::isnan(config.inputShapeInitVels[i][0][1]) &&
              !std::isnan(config.inputShapeInitVels[i][0][2]) &&
              !std::isnan(config.inputShapeInitVels[i][1][0]) &&
              !std::isnan(config.inputShapeInitVels[i][1][1]) &&
              !std::isnan(config.inputShapeInitVels[i][1][2])) {
            Eigen::Vector3i startToEnd;
            startToEnd[0] = i;
            startToEnd[1] = V.rows();
            startToEnd[2] = V.rows() + newV.rows();
            componentInitVels.emplace_back(startToEnd,
                                           config.inputShapeInitVels[i]);
          }
          ...
          while (DBCI < config.inputShapeDBC.size() &&
                 config.inputShapeDBC[DBCI].first == i) {
            // vertex selection
            std::vector<int> selectedVerts;
            const auto &inputDBC = config.inputShapeDBC[DBCI].second;
            IPC::IglUtils::Init_Dirichlet(newV, inputDBC.minBBox,
                                          inputDBC.maxBBox, isNodeOnBoundary,
                                          selectedVerts);
            for (auto &i : selectedVerts) {
              i += V.rows();
            }
            if (selectedVerts.size()) {
              DirichletBCs.emplace_back(selectedVerts, inputDBC.linearVelocity,
                                        inputDBC.angularVelocity,
                                        inputDBC.timeRange);
            }
            ++DBCI;
          }
          while (NBCI < config.inputShapeNBC.size() &&
                 config.inputShapeNBC[NBCI].first == i) {
            // vertex selection
            std::vector<int> selectedVerts;
            const auto &inputNBC = config.inputShapeNBC[NBCI].second;
            IPC::IglUtils::Init_Dirichlet(newV, inputNBC.minBBox,
                                          inputNBC.maxBBox, isNodeOnBoundary,
                                          selectedVerts);
            for (auto &i : selectedVerts) {
              i += V.rows();
            }
            if (selectedVerts.size()) {
              NeumannBCs.emplace_back(selectedVerts, inputNBC.force,
                                      inputNBC.timeRange);
            }
            ++NBCI;
          }
        ...
        for (int i = 0; i < (int)newV.rows(); ++i) {
          ...
          newV.row(i) =
              (rotate * p.cwiseProduct(scale) + translate).transpose();
        }
      ...
      if (config.rotDeg != 0.0) {
        const Eigen::Matrix3d rotMtr =
            Eigen::AngleAxis<double>(config.rotDeg / 180.0 * M_PI,
                                     config.rotAxis)
                .toRotationMatrix();
        Eigen::Matrix<double, 1, DIM> center =
            (UV.colwise().maxCoeff() - UV.colwise().minCoeff()) / 2.0;
#ifdef USE_TBB
        tbb::parallel_for(
            0, (int)UV.rows(), 1,
            [&](int vI)
        ...
        for (int vI = 0; vI < UV.rows(); ++vI)
#endif
            {
              if constexpr (DIM == 3) {
                UV.row(vI) =
                    (rotMtr * (UV.row(vI) - center).transpose()).transpose() +
                    center;
              } else {
                Eigen::Vector3d pos;
      ...
      if (config.size > 0.0) {
        V *= config.size /
             (UV.colwise().maxCoeff() - UV.colwise().minCoeff()).maxCoeff();
        UV *= config.size /
              (UV.colwise().maxCoeff() - UV.colwise().minCoeff()).maxCoeff();
      ...
      for (int vI = 0; vI < V.rows(); ++vI)
#endif
          {
            // nonuniform scale
            UV(vI, 0) *=
                1.0 + 0.1 * (UV(vI, 0) - minCoords[0]) / coordsRange[0];

            double yRatio = (UV(vI, 1) - minCoords[1]) / coordsRange[1];
            UV(vI, 1) += yRatio * 0.1 * UV(vI, 2);

            UV(vI, 2) *=
                1.0 - 0.1 * (UV(vI, 2) - minCoords[2]) / coordsRange[2];
          }
#ifdef USE_TBB
      );
#endif
      ...
      IPC::IglUtils::findBorderVerts(V, borderVerts_primitive,
                                     config.handleRatio);
  ...
  IPC::Mesh<DIM> *temp = new IPC::Mesh<DIM>(
      V, F, SF, E, UV, componentNodeRange, componentSFRange, componentCERange,
      componentCoDim, componentMaterial, componentLVels, componentAVels,
      componentInitVels, DirichletBCs, NeumannBCs,
      config.inputShapeMeshSeqFolderPath, config.YM, config.PR, config.rho);
  ...
  for (int coI = 0; coI < config.collisionObjects.size(); ++coI) {
    config.collisionObjects[coI]->saveMesh(outputFolderPath + "/ACO" +
                                               std::to_string(coI) + "_0.obj",
                                           IPC::invShift);
  }
  for (int mcoI = 0; mcoI < config.meshCollisionObjects.size(); ++mcoI) {
    config.meshCollisionObjects[mcoI]->saveMesh(
        outputFolderPath + "/MCO" + std::to_string(mcoI) + "_0.obj",
        IPC::invShift);
  }
  ...
  if (!logFile.is_open()) {
    spdlog::error("failed to create log file, please ensure output directory "
  ...
  optimizer = new IPC::Optimizer<DIM>(
      *triSoup[0], energyTerms, energyParams, false, Eigen::MatrixXd(),
      Eigen::MatrixXi(), Eigen::VectorXi(), config);
  ...
  GIFDelay = static_cast<int>(delay_10ms *
                              GIFStep); // always about 3x10ms, around 33FPS
  // set slower motion playback
  GIFStep = static_cast<int>(GIFStep * config.playBackSpeed);
  if (GIFStep < 1) {
    GIFStep = 1;
    logFile << config.playBackSpeed
            << "X playback can't be reached with 33FPS at dt="
            << optimizer->getDt() << std::endl;
    logFile << "using playBackSpeed at "
            << 1.0 / static_cast<int>(std::ceil(3.0 / delay_10ms)) << "X"
            << std::endl;
  }
  ...
  const static Eigen::IOFormat CSVFormat(Eigen::FullPrecision,
                                         Eigen::DontAlignCols, ", ", "\n");
  std::ofstream file(outputFolderPath + "/SF");
  if (file.is_open()) {
    file << SF.format(CSVFormat);
    file.close();
  }

  // Save files for testing TODO:Delete this when development is finished
  std::ofstream file2(outputFolderPath + "/V_rest");
  if (file2.is_open()) {
    file2 << V.format(CSVFormat);
    file2.close();
  }

  std::ofstream file3(outputFolderPath + "/F");
  if (file3.is_open()) {
    file3 << F.format(CSVFormat);
    file3.close();
  }
  ...
  if (offlineMode) {
    ...
    viewer.launch();
#else
    spdlog::error("Only offline mode is supported when OpenGL is disabled. See "
