...
enum EnergyType { ET_NH, ET_FCR };

enum TimeIntegrationType { TIT_BE, TIT_NM };

enum TimeStepperType { TST_NEWTON };

enum ConstraintSolverType { CST_QP, CST_SQP, CST_IP };

enum QPSolverType { QP_OSQP, QP_GUROBI };
...
struct InputDBC {
  InputDBC(const Eigen::Vector3d &minBBox, const Eigen::Vector3d &maxBBox,
      ...
      : minBBox(minBBox), maxBBox(maxBBox), linearVelocity(linearVelocity),
        angularVelocity(angularVelocity), timeRange(timeRange) {}
...
public:
  ...
  std::array<double, 2> DBCTimeRange = {
      0.0, std::numeric_limits<double>::infinity()};
  std::array<double, 2> NBCTimeRange = {
      0.0, std::numeric_limits<double>::infinity()};
  ...
  /// @brief STRUMPACK parameters
  std::string Compression;

  /// @brief simulation name
  std::string sim_name;

  /// @brief always do analysis
  bool doAnalysis;

  /// @brief Ordering Method
  std::string orderingMethod;

  /// @brief apply PARTH_LAG
  bool parth_lag;

  /// @brief apply aggressive reuse
...
public:
  ...
  static TimeIntegrationType
  getTimeIntegrationTypeByStr(const std::string &str);
  static std::string
  getStrByTimeIntegrationType(TimeIntegrationType timeIntegrationType);
  static LinSysSolverType getLinSysSolverTypeByStr(const std::string &str);
  static ConstraintSolverType
  getConstraintSolverTypeByStr(const std::string &str);
  static std::string
  getStrByConstraintSolverType(ConstraintSolverType constraintSolverType);
