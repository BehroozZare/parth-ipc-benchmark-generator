...
void AnimScripter<dim>::setDBCVertices(Mesh<dim> &mesh) const {
  ...
  for (int compI = 0; compI < mesh.componentCoDim.size(); ++compI) {
    if (mesh.componentCoDim[compI] < 3) {
      for (int i = mesh.componentNodeRange[compI];
           i < mesh.componentNodeRange[compI + 1]; ++i) {
  ...
  for (const auto &scriptedLVelI : mesh.componentLVels) {
    DirichletBCType type = scriptedLVelI.second.isZero()
                               ? DirichletBCType::ZERO
                               : DirichletBCType::NONZERO;
    for (int i = scriptedLVelI.first[1]; i < scriptedLVelI.first[2]; ++i) {
      updateType(i, type);
    }
  }
  for (const auto &scriptedAVelI : mesh.componentAVels) {
    DirichletBCType type = scriptedAVelI.second.isZero()
                               ? DirichletBCType::ZERO
                               : DirichletBCType::NONZERO;
  ...
  if (stepStartTime >= DBCTimeRange[0] && stepStartTime < DBCTimeRange[1]) {
    for (const auto &DBC : mesh.DirichletBCs) {
      DirichletBCType type =
          DBC.isZero() ? DirichletBCType::ZERO : DirichletBCType::NONZERO;
      if (stepStartTime >= DBC.timeRange[0] &&
          stepStartTime < DBC.timeRange[1]) {
...
template <int dim>
void AnimScripter<dim>::initAnimScript(
    Mesh<dim> &mesh,
  ...
  case AST_SCALEF: {
    ...
    // 0.0, 0.5, -0.5,
    // 0.0, 0.0, 1.0;
    M << 1.5, 0.0, 0.0, 0.0, 1.5, 0.0, 0.0, 0.0, 1.5;
  ...
  case AST_HANGTOPLEFT: {
    ...
    for (const auto &vI : mesh.borderVerts_primitive[0]) {
      if (mesh.V(vI, 1) > topRight[1] - range[1] * 0.01 &&
          (mesh.V(vI, 2) > topRight[2] - range[2] * 0.01 ||
           mesh.V(vI, 2) < bottomLeft[2] + range[2] * 0.01)) {
        mesh.addDBCVertex(vI, DirichletBCType::ZERO);
      }
    }
    break;
  ...
  case AST_SWING: {
    mesh.V.col(1).array() +=
        1.3 * (mesh.V.col(1).maxCoeff() - mesh.V.col(1).minCoeff());
  ...
  case AST_STAMP:
    mesh.resetDBCVertices();
    mesh.addDBCVertices(mesh.borderVerts_primitive[0],
                        DirichletBCType::NONZERO);
    break;
  ...
  case AST_STAMPBOTH:
    ...
    mesh.addDBCVertices(mesh.borderVerts_primitive[0],
                        DirichletBCType::NONZERO);
    mesh.addDBCVertices(mesh.borderVerts_primitive[1],
                        DirichletBCType::NONZERO);
    break;
  ...
  case AST_UNDSTAMP:
    ...
    velocityTurningPoints.second(1, 0) =
        mesh.V(velocityTurningPoints.first, 1) - 0.6;
    velocityTurningPoints.second(1, 1) =
        mesh.V(velocityTurningPoints.first, 1) + 0.6;
    break;
  ...
  case AST_BEND: {
    ...
    for (const auto borderI : mesh.borderVerts_primitive) {
      ...
      for (int bVI = 0; bVI + 1 < borderI.size(); bVI++) {
        ...
        rotCenter_handleVerts[borderI[bVI]] =
            mesh.V.row(borderI.back()).transpose();
      }
      bI++;
    }
    break;
  ...
  case AST_TWIST: {
    ...
    for (const auto borderI : mesh.borderVerts_primitive) {
      ...
      for (int bVI = 0; bVI < borderI.size(); bVI++) {
        ...
        rotCenter_handleVerts[borderI[bVI]] =
            rotCenter.transpose().topRows(dim);
      }
      bI++;
    }
    break;
  ...
  case AST_FOURLEGPULL: {
    ...
    bool turningPointSet = false;
    for (int vI = 0; vI < mesh.V.rows(); ++vI) {
      if ((mesh.V(vI, 1) > topRight[1] - range[1] * 0.129) &&
          ((mesh.V(vI, 0) < bottomLeft[0] + range[0] * 0.16))) {
        // left top
        mesh.addDBCVertex(vI, DirichletBCType::NONZERO);
        handleVerts[1].emplace_back(vI);
        velocity_handleVerts[vI].setZero();
      } else if ((mesh.V(vI, 1) > topRight[1] - range[1] * 0.16) &&
                 (mesh.V(vI, 0) > topRight[0] - range[0] * 0.16)) {
        // right top
        mesh.addDBCVertex(vI, DirichletBCType::NONZERO);
        handleVerts[0].emplace_back(vI);
        velocity_handleVerts[vI].setZero();
        velocity_handleVerts[vI][0] = 2.5;
      } else if ((mesh.V(vI, 1) < bottomLeft[1] + range[1] * 0.02) &&
                 (mesh.V(vI, 0) > topRight[0] - range[0] * 0.25)) {
        // bottom right
        mesh.addDBCVertex(vI, DirichletBCType::NONZERO);
        handleVerts[0].emplace_back(vI);
        velocity_handleVerts[vI].setZero();
        ...
        velocity_handleVerts[vI][1] = -3.5;
                 ...
                 (mesh.V(vI, 0) < bottomLeft[0] + range[0] * 0.25)) {
        // bottom left
        mesh.addDBCVertex(vI, DirichletBCType::NONZERO);
        handleVerts[0].emplace_back(vI);
        velocity_handleVerts[vI].setZero();
  ...
  case AST_HEADTAILPULL: {
    ...
    for (int vI = 0; vI < mesh.V.rows(); ++vI) {
      if (mesh.V(vI, 2) < bottomLeft[2] + range[2] * 0.02) {
        ...
        velocity_handleVerts[vI].setZero();
        velocity_handleVerts[vI][0] = 3.5;
      } else if ((mesh.V(vI, 2) > bottomLeft[2] + range[2] * 0.46) &&
                 (mesh.V(vI, 2) < bottomLeft[2] + range[2] * 0.54)) {
        // bottom right
        mesh.addDBCVertex(vI, DirichletBCType::NONZERO);
        handleVerts[1].emplace_back(vI);
        velocity_handleVerts[vI].setZero();
  ...
  case AST_FALL: {
    mesh.V.col(1).array() +=
        0.5 *
        (mesh.V.colwise().maxCoeff() - mesh.V.colwise().minCoeff()).norm();
    mesh.resetDBCVertices();
    break;
  }
  ...
  case AST_DRAGDOWN: {
    ...
    Eigen::RowVectorXd range = topRight - bottomLeft;
    for (int vI = 0; vI < mesh.V.rows(); ++vI) {
      if ((mesh.V(vI, 1) < bottomLeft[1] + range[1] * 0.1) &&
          (mesh.V(vI, 0) < bottomLeft[0] + range[0] * 0.52) &&
          (mesh.V(vI, 0) > bottomLeft[0] + range[0] * 0.42)) {
        // bottom middle
        mesh.addDBCVertex(vI, DirichletBCType::NONZERO);
        velocity_handleVerts[vI].setZero();
  ...
  case AST_DRAGRIGHT: {
    ...
    for (int vI = 0; vI < mesh.V.rows(); ++vI) {
      if (mesh.V(vI, 0) >
          topRight[0] -
              range[0] * 0.04) { // 0.001 for dolphin rightmost 2 nodes
        mesh.addDBCVertex(vI, DirichletBCType::NONZERO);
        velocity_handleVerts[vI].setZero();
  ...
  case AST_CURTAIN: {
    ...
    for (int vI = 0; vI < mesh.V.rows(); ++vI) {
      for (int pinI = 0; pinI < 8; ++pinI) {
        if (mesh.V(vI, 0) >
                bottomLeft[0] + range[0] / 7.0 * pinI - range[0] * 0.0025 &&
            mesh.V(vI, 0) <
                bottomLeft[0] + range[0] / 7.0 * pinI + range[0] * 0.0025 &&
            mesh.V(vI, 1) > topRight[1] - range[1] * 0.005) {
          mesh.addDBCVertex(vI, DirichletBCType::NONZERO);
          velocity_handleVerts[vI].setZero();
  ...
  case AST_MCOROTSQUASH: {
    ...
    MCORotCenter[0][1] +=
        std::dynamic_pointer_cast<MeshCO<dim>>(MCO[0])->scale / 2.0;
    MCORotCenter[1] = MCO[1]->origin;
    MCORotCenter[1][1] -=
        std::dynamic_pointer_cast<MeshCO<dim>>(MCO[1])->scale / 2.0;
  ...
  case AST_DCOROTCYLINDERS: {
    ...
    for (int compI = 0; compI < 4; ++compI) {
      ...
      MCORotCenter[compI] =
          0.5 * (mesh.V
                     .block(mesh.componentNodeRange[compI], 0,
                            mesh.componentNodeRange[compI + 1] -
                                mesh.componentNodeRange[compI],
                            3)
                     .colwise()
                     .maxCoeff() +
                 mesh.V
  ...
  case AST_DCOHAMMERWALNUT: {
    ...
    for (int i = mesh.componentNodeRange[1]; i < mesh.componentNodeRange[2];
         ++i) {
      DBCVerts.emplace_back(i);
  ...
  case AST_DCOSQUEEZEOUT: {
    std::cout
        << "HELOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO with one L"
        << std::endl;
    mesh.resetDBCVertices();
    std::vector<int> DBCVerts;
    for (int compI = 0; compI < mesh.componentCoDim.size(); ++compI) {
    ...
    for (int vI = 0; vI < mesh.V.rows(); ++vI) {
      if (mesh.V(vI, 1) > topRight[1] - range[0] * 1e-4) { // isOnTop
        NBCVertices.push_back(vI);
                 ...
                 bottomLeft[1] + range[0] * 1e-4) { // isOnBottom
        mesh.addDBCVertex(vI, DirichletBCType::NONZERO);
      }
    }
      ...
      if (mesh.componentCoDim[compI] == dim) {
        ...
        for (int vI = initVelInfo.first[1]; vI < initVelInfo.first[2]; ++vI) {
          if (!mesh.isDBCVertex(vI)) {
            velocity.template segment<dim>(vI * dim) =
                initVelInfo.second[0] +
                initVelInfo.second[1].cross(
                    Eigen::Vector3d(mesh.V.row(vI).transpose()) - rotCenter);
          }
        }
      }
    }
...
template <int dim>
int AnimScripter<dim>::stepAnimScript(
    Mesh<dim> &mesh, const SpatialHash<dim> &sh,
    std::vector<std::shared_ptr<CollisionObject<dim>>> &ACO,
    std::vector<std::shared_ptr<CollisionObject<dim>>> &MCO,
    const ccd::CCDMethod method, double dt, double dHat,
    const std::vector<Energy<dim> *> &energyTerms, bool isSelfCollision,
    bool forceIntersectionLineSearch) {
  ...
  case AST_NULL: {
    ...
    for (const auto &scriptedAVelI : mesh.componentAVels) {
      Eigen::Matrix3d rotMtr = (Eigen::AngleAxisd(scriptedAVelI.second[0] * dt,
                                                  Eigen::Vector3d::UnitX()) *
                                Eigen::AngleAxisd(scriptedAVelI.second[1] * dt,
                                                  Eigen::Vector3d::UnitY()) *
                                Eigen::AngleAxisd(scriptedAVelI.second[2] * dt,
                                                  Eigen::Vector3d::UnitZ()))
                                   .toRotationMatrix();

      int compI = scriptedAVelI.first[0];
      Eigen::Vector3d MCORotCenter =
          0.5 * (mesh.V
                     .block(mesh.componentNodeRange[compI], 0,
                            mesh.componentNodeRange[compI + 1] -
                                mesh.componentNodeRange[compI],
      ...
      for (int vI = scriptedAVelI.first[1]; vI < scriptedAVelI.first[2]; ++vI) {
        searchDir.template segment<dim>(vI * dim) +=
            rotMtr.template block<dim, dim>(0, 0) *
                (mesh.V.row(vI).transpose() - MCORotCenter) +
            MCORotCenter - mesh.V.row(vI).transpose();
      }
    }
    ...
    if (stepStartTime >= DBCTimeRange[0] && stepStartTime < DBCTimeRange[1]) {
      for (const auto &DBC : mesh.DirichletBCs) {
        ...
        Eigen::Matrix3d rotMtr = (Eigen::AngleAxisd(DBC.angularVelocity[0] * dt,
                                                    Eigen::Vector3d::UnitX()) *
                                  Eigen::AngleAxisd(DBC.angularVelocity[1] * dt,
                                                    Eigen::Vector3d::UnitY()) *
                                  Eigen::AngleAxisd(DBC.angularVelocity[2] * dt,
                                                    Eigen::Vector3d::UnitZ()))
                                     .toRotationMatrix();

        Eigen::RowVector3d min = mesh.V.row(DBC.vertIds[0]),
                           max = mesh.V.row(DBC.vertIds[0]);
        for (const auto &vI : DBC.vertIds) {
        ...
        for (const auto &vI : DBC.vertIds) {
          searchDir.template segment<dim>(vI * dim) +=
              rotMtr.template block<dim, dim>(0, 0) *
                  (mesh.V.row(vI).transpose() - rotCenter) +
              rotCenter + DBC.linearVelocity * dt - mesh.V.row(vI).transpose();
        }
      }
    }
    ...
    for (const auto &meshSeqI : mesh.meshSeqFolderPath) {
      ...
      case 3: {
        ...
        if (!IglUtils::readTetMesh(
                fmt::format("{}/{:d}.msh", meshSeqI.second, meshI), Vms, Fms,
                SFms)) {
          spdlog::error("Unable to read input file: {}/{:d}.msh",
                        meshSeqI.second, meshI);
          exit(-1);
        }
        break;
      }
      ...
      case 2: {
        // obj
        Eigen::MatrixXi Fms;
        if (!igl::readOBJ(fmt::format("{}/{:d}.obj", meshSeqI.second, meshI),
                          Vms, Fms)) {
          spdlog::error("Unable to read input file: {}/{:d}.obj",
                        meshSeqI.second, meshI);
          exit(-1);
        }
        break;
      ...
      case 1: {
        ...
        Eigen::MatrixXi Ems;
        if (!IglUtils::readSEG(
                fmt::format("{}/{:d}.seg", meshSeqI.second, meshI), Vms, Ems)) {
          // if not found, from obj
          Eigen::MatrixXi Fms;
          if (!igl::readOBJ(fmt::format("{}/{:d}.obj", meshSeqI.second, meshI),
                            Vms, Fms)) {
            spdlog::error("Unable to read input file: {}/{:d}.seg or .obj",
                          meshSeqI.second, meshI);
            exit(-1);
          }
        }
      ...
      case 0: {
        // pt
        Eigen::MatrixXi Fms;
        if (!igl::readOBJ(fmt::format("{}/{:d}.pt", meshSeqI.second, meshI),
                          Vms, Fms)) {
          // if not found, from obj
          if (!igl::readOBJ(fmt::format("{}/{:d}.obj", meshSeqI.second, meshI),
                            Vms, Fms)) {
            spdlog::error("Unable to read input file: {}/{:d}.pt or .obj",
                          meshSeqI.second, meshI);
            exit(-1);
          }
        }
      ...
      if (Vms.rows() !=
          mesh.componentNodeRange[compI + 1] - mesh.componentNodeRange[compI]) {
        spdlog::warn("meshSeq #V does not match with mesh!");
        exit(-1);
      }
      for (int vI = mesh.componentNodeRange[compI];
           vI < mesh.componentNodeRange[compI + 1]; ++vI) {
        searchDir.template segment<dim>(vI * dim) =
            (Vms.row(vI - mesh.componentNodeRange[compI]) - mesh.V.row(vI))
                .transpose();
      }
    }
  ...
  case AST_PUSH: {
    if (mesh.V(velocityTurningPoints.first, 1) <=
        velocityTurningPoints.second(0, 1)) {
      velocityTurningPoints.second(0, 1) =
  ...
  case AST_UPNDOWN: {
    ...
    if ((mesh.V(velocityTurningPoints.first, 1) <=
         velocityTurningPoints.second(1, 0)) ||
        (mesh.V(velocityTurningPoints.first, 1) >=
         velocityTurningPoints.second(1, 1))) {
      flip = true;
    }
  ...
  case AST_BEND:
    for (const auto &movingVerts : angVel_handleVerts) {
      ...
      searchDir.segment<dim>(movingVerts.first * dim) =
          (rotMtr.block<dim, dim>(0, 0) *
               (mesh.V.row(movingVerts.first).transpose() - rotCenter->second) +
           rotCenter->second) -
          mesh.V.row(movingVerts.first).transpose();
    }
    break;
  ...
  case AST_ACOSQUASH6: {
    ...
    for (int acoI = 0; acoI < ACOStepSizeLeft.size(); ++acoI) {
      ACO[acoI]->move(ACOVelocity[acoI] * dt, mesh, sh, 0.5,
                      ACOStepSizeLeft[acoI]);
    }
    break;
  }
  ...
  case AST_MCOROTSQUASH: {
    ...
    for (int coI = 0; coI < MCO.size(); ++coI) {
      ...
      Eigen::Matrix3d rotMtr =
          (Eigen::AngleAxisd(MCOAngVel[coI][0] * dt, Eigen::Vector3d::UnitX()) *
      ...
      for (int vI = 0; vI < MCO[coI]->V.rows(); ++vI) {
        MCO[coI]->V_target.row(vI) =
            (rotMtr.template block<dim, dim>(0, 0) *
                 (MCO[coI]->V.row(vI).transpose() - MCORotCenter[coI]) +
             MCORotCenter[coI])
                .transpose();
      }
      MCOVelocity[coI] = (rotMtr.template block<dim, dim>(0, 0) *
                              (MCO[coI]->origin - MCORotCenter[coI]) +
                          MCORotCenter[coI] - MCO[coI]->origin) /
                         dt;
    }
  ...
  case AST_DCOROTCYLINDERS: {
    ...
    for (int compI = 0; compI < 4; ++compI) {
      Eigen::Matrix3d rotMtr = (Eigen::AngleAxisd(MCOAngVel[compI][0] * dt,
                                                  Eigen::Vector3d::UnitX()) *
                                Eigen::AngleAxisd(MCOAngVel[compI][1] * dt,
                                                  Eigen::Vector3d::UnitY()) *
                                Eigen::AngleAxisd(MCOAngVel[compI][2] * dt,
                                                  Eigen::Vector3d::UnitZ()))
                                   .toRotationMatrix();
      for (int vI = mesh.componentNodeRange[compI];
           vI < mesh.componentNodeRange[compI + 1]; ++vI) {
        searchDir.template segment<dim>(vI * dim) =
            rotMtr.template block<dim, dim>(0, 0) *
                (mesh.V.row(vI).transpose() - MCORotCenter[compI]) +
            MCORotCenter[compI] - mesh.V.row(vI).transpose();
      }
    }
    break;
  }
  ...
  case AST_DCOHAMMERWALNUT: {
    ...
    for (int vI = mesh.componentNodeRange[1]; vI < mesh.componentNodeRange[2];
         ++vI) {
      if (yMin > mesh.V(vI, 1)) {
        yMin = mesh.V(vI, 1);
      }
    ...
    if (yMin > 0.05) {
      Eigen::Matrix3d rotMtr =
          (Eigen::AngleAxisd(MCOAngVel[0][0] * dt, Eigen::Vector3d::UnitX()) *
      ...
      for (int vI = mesh.componentNodeRange[1]; vI < mesh.componentNodeRange[2];
        ...
        searchDir.template segment<dim>(vI * dim) =
            rotMtr.template block<dim, dim>(0, 0) *
                (mesh.V.row(vI).transpose() - MCORotCenter[0]) +
            MCORotCenter[0] - mesh.V.row(vI).transpose();
      }
    }
  ...
  case AST_DCOSQUASH: {
    ...
    double co0right =
        mesh.V.col(0)
            .segment(mesh.componentNodeRange[0],
                     mesh.componentNodeRange[1] - mesh.componentNodeRange[0])
            .maxCoeff();
    double co1left =
        mesh.V.col(0)
            .segment(mesh.componentNodeRange[1],
                     mesh.componentNodeRange[2] - mesh.componentNodeRange[1])
            .minCoeff();
  ...
  case AST_DCOSEGBEDSQUASH: {
    double topMin = std::numeric_limits<double>::infinity(),
           bottomMax = -std::numeric_limits<double>::infinity();
    for (int compI = 0; compI < mesh.componentCoDim.size(); ++compI) {
      if (mesh.componentCoDim[compI] < 3) {
        if (compI >= (mesh.componentCoDim.size() + 1) / 2) {
          topMin =
              std::min(topMin, mesh.V.col(1)
                                   .segment(mesh.componentNodeRange[compI],
                                            mesh.componentNodeRange[compI + 1] -
                                                mesh.componentNodeRange[compI])
                                   .minCoeff());
        } else {
          bottomMax = std::max(bottomMax,
                               mesh.V.col(1)
                                   .segment(mesh.componentNodeRange[compI],
                                            mesh.componentNodeRange[compI + 1] -
                                                mesh.componentNodeRange[compI])
  ...
  case AST_DCOSQUEEZEOUT: {
    ...
    for (int compI = 0; compI < mesh.componentCoDim.size(); ++compI) {
      if (mesh.componentCoDim[compI] < 3) {
        if (compI == 0) {
          topMax =
              std::max(topMax, mesh.V.col(1)
                                   .segment(mesh.componentNodeRange[compI],
                                            mesh.componentNodeRange[compI + 1] -
                                                mesh.componentNodeRange[compI])
                                   .maxCoeff());
        } else if (compI == 1) {
          bottomMin = std::min(bottomMin,
                               mesh.V.col(1)
                                   .segment(mesh.componentNodeRange[compI],
                                            mesh.componentNodeRange[compI + 1] -
                                                mesh.componentNodeRange[compI])
  ...
  case AST_MESHSEQ_FROMFILE: {
    ...
    Eigen::MatrixXd Vms;
    Eigen::MatrixXi Fms;
    igl::readOBJ(meshSeqFolderPath + "/" + std::to_string(meshI) + ".obj", Vms,
                 Fms);
    for (int compI = 0; compI < mesh.componentCoDim.size(); ++compI) {
      if (mesh.componentCoDim[compI] < 3) {
        if (Vms.rows() != mesh.componentNodeRange[compI + 1] -
                              mesh.componentNodeRange[compI]) {
  ...
  for (int vI = 0; vI < mesh.V.rows(); ++vI) {
    if (mesh.isDBCVertex(vI) ||
        !searchDir.template segment<dim>(vI * dim).isZero()) {
      targetPos[vI].first =
          mesh.V.row(vI) +
          searchDir.template segment<dim>(vI * dim).transpose();
  ...
  if (isSelfCollision &&
      !searchDir.isZero()) { // assume MBC does not collide with any other CO
#ifdef USE_SH_LFSS
    ...
    if (method == ccd::CCDMethod::FLOATING_POINT_ROOT_FINDER) {
      ...
      SelfCollisionHandler<dim>::largestFeasibleStepSize_CCD(
          mesh, sh_CCD, searchDir, 0.5, newCandidates, stepSize);
    } else {
      SelfCollisionHandler<dim>::largestFeasibleStepSize_CCD_exact(
          mesh, sh_CCD, searchDir, method, stepSize);
    }
  }

#ifdef OUTPUT_CCD_FAIL
  bool output = false;
  if (isSelfCollision && !searchDir.isZero() &&
      method != ccd::CCDMethod::FLOATING_POINT_ROOT_FINDER) {
    mesh.saveSurfaceMesh(outputFolderPath + "before" +
  ...
  if (!forceIntersectionLineSearch) {
    ...
    isInitSelfIntersecting =
        !SelfCollisionHandler<dim>::checkEdgeTriIntersectionIfAny(mesh, sh_CCD);
  }

  // Move the vertices
  Eigen::MatrixXd V0 = mesh.V;
#ifdef USE_TBB
  tbb::parallel_for(0, (int)mesh.V.rows(), 1,
                    [&](int vI)
#else
  for (int vI = 0; vI < mesh.V.rows(); ++vI)
#endif
  ...
  if (energyTerms[0]->getNeedElemInvSafeGuard() && !searchDir.isZero()) {
    while (!mesh.checkInversion(true)) {
      logFile << "element inversion detected during step anim, backtrack!"
              << std::endl;
      stepSize /= 2.0;
#ifdef USE_TBB
      tbb::parallel_for(0, (int)mesh.V.rows(), 1,
  ...
  // Line-search to prevent intersections (skip if there are already
  // intersections)
  if ((forceIntersectionLineSearch || !isInitSelfIntersecting) &&
      isSelfCollision && !searchDir.isZero()) {
    sh_CCD.build(mesh, mesh.avgEdgeLen / 3.0);
    while (!SelfCollisionHandler<dim>::checkEdgeTriIntersectionIfAny(mesh,
                                                                     sh_CCD)) {
#ifdef OUTPUT_CCD_FAIL
      if (output) {
        mesh.saveSurfaceMesh(outputFolderPath + "after" +
                             std::to_string(numOfCCDFail) + ".obj");
        ++numOfCCDFail;
        output = false;
      }
#endif
      logFile << "intersection detected during step anim, backtrack!"
              << std::endl;
      stepSize /= 2.0;
#ifdef USE_TBB
      tbb::parallel_for(0, (int)mesh.V.rows(), 1,
  ...
  SelfCollisionHandler<dim>::largestFeasibleStepSize_CCD_exact(
      mesh, sh_CCD, searchDir, ccd::CCDMethod::RATIONAL_ROOT_PARITY,
      stepSize_rational);
...
template <int dim> double AnimScripter<dim>::getCompletedStepSize(void) const {
  return stepSize;
}
...
template <int dim>
void AnimScripter<dim>::augmentMDBCEnergy(const Mesh<dim> &data, double &E,
                                          double rho) const {
  for (const auto &tpI : targetPos) {
    int vI = tpI.first;
    E -=
        std::sqrt(data.massMatrix.coeff(vI, vI)) *
        tpI.second.second.cwiseProduct(data.V.row(vI) - tpI.second.first).sum();
    E += rho / 2.0 * data.massMatrix.coeff(vI, vI) *
         (data.V.row(vI) - tpI.second.first).squaredNorm();
  }
}

template <int dim>
void AnimScripter<dim>::augmentMDBCGradient(const Mesh<dim> &data,
                                            Eigen::VectorXd &g,
                                            double rho) const {
  for (const auto &tpI : targetPos) {
    int vI = tpI.first;
    g.template segment<dim>(vI * dim) -=
        std::sqrt(data.massMatrix.coeff(vI, vI)) *
        tpI.second.second.transpose();
    g.template segment<dim>(vI * dim) +=
        rho * data.massMatrix.coeff(vI, vI) *
        (data.V.row(vI) - tpI.second.first).transpose();
  }
}

template <int dim>
void AnimScripter<dim>::augmentMDBCHessian(
    const Mesh<dim> &data,
...
void AnimScripter<dim>::updateLambda(const Mesh<dim> &data, double rho) {
  for (auto &tpI : targetPos) {
    int vI = tpI.first;
    tpI.second.second -= rho * std::sqrt(data.massMatrix.coeff(vI, vI)) *
                         (data.V.row(vI) - tpI.second.first);
  }
}

template <int dim> void AnimScripter<dim>::resetLambda(void) {
  for (auto &tpI : targetPos) {
...
template <int dim> bool AnimScripter<dim>::isNBCActive(void) const {
...
bool AnimScripter<dim>::isNBCActive(const Mesh<dim> &mesh, int NBCi) const {
  return stepStartTime >= mesh.NeumannBCs[NBCi].timeRange[0] &&
         stepStartTime < mesh.NeumannBCs[NBCi].timeRange[1];
}
...
template <int dim>
void AnimScripter<dim>::setMeshSeqFolderPath(
    const std::string &p_meshSeqFolderPath) {
  meshSeqFolderPath = p_meshSeqFolderPath;
}

template <int dim>
AnimScriptType
AnimScripter<dim>::getAnimScriptTypeByStr(const std::string &str) {
...
template <int dim>
std::string
AnimScripter<dim>::getStrByAnimScriptType(AnimScriptType animScriptType) {
