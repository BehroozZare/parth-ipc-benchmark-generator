...
def requiresFormat(FileName):
    ...
    if FormattedContent == FileContent:
        return False
    print("Formatted content differs for file", FileName)
    print(type(FileContent.splitlines(1)[0]))
    print(type(FormattedContent.splitlines(1)[0]))
    file_contect_str = [x.decode('utf-8') for x in FileContent.splitlines(1)] 
    format_content_str = [x.decode('utf-8') for x in FormattedContent.splitlines(1)]
    # Diff = difflib.unified_diff(FileContent.splitlines(1), FormattedContent.splitlines(1),
    #                             fromfile=FileName, tofile=FileName)
    Diff = difflib.unified_diff(file_contect_str, format_content_str,
                            fromfile=FileName, tofile=FileName)
    for line in Diff:
